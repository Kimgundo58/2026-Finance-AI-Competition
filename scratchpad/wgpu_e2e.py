...
def main() -> int:
    ...
    보고: dict = {"경로": a.경로, "요청": 정규화본문, "org_id": ORG, "사업명": 사업}
    ...
    ev1 = sse(f"/api/normalize?org_id={ORG}", 정규화본문, f"normalize_{a.경로}")
    p1 = 프록시수()
    m1 = 메트릭수()
    ...
    if not isinstance(결과1, dict):
        ...
        (OUT / f"_보고_{a.경로}.json").write_text(
    ...
    판정본문 = {"정규화": {**결과1, "_원문": 질문}, "확정비목": 확정,
             "사업명": 사업, "org_id": ORG}
    ev2 = sse(f"/api/judge?org_id={ORG}", 판정본문, f"judge_{a.경로}")
    p2 = 프록시수()
    m2 = 메트릭수()
    ...
    print(f"  보고 scratchpad/_보고_{a.경로}.json", flush=True)
    return 0
