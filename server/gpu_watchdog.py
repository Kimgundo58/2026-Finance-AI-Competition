    ...
    def 현황(self) -> dict:
        ...
        with self._락:
            ...
            return {
                ...
                "vLLM_응답": vllm_응답,
                # 🔴 2026-09-06(레인 D) 신설 — 「상태:가동」이 «실제 조회 성공» 인지
                # «모르는 걸 가동으로 접은 것» 인지 갈라준다. 위 :396 주석이 스스로
                # 지적한 사각지대다: 팟이 «아예 없어도»(RunPod 계정 팟 0개·404) 조회는
                # `알수없음` 이 되고, 그게 「상태:가동」으로 나갔다. 어휘를 늘리지 않고
                # (프론트 파서를 안 깨고) «사실 하나» 만 더한다 — false 면 위 「상태」
                # 필드를 «믿지 말라» 는 뜻이다. 참고: `종료예정초 is None` 은 이미
                # 있던 신호이지만 그건 "끌 계획 없음"이 원인일 수도 있어 알수없음과
                # 안 겹친다 — 그래서 새 필드가 필요했다.
