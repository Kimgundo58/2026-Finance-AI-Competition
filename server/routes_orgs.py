...
def _샘플계획() -> list[tuple]:
    ...
    `scripts/archive/seed/seed_demo.py` 도 같은 곳에서 읽는다. 두 군데에 따로 적으면 갈린다.
...
def _데모사업명들() -> list[str]:
    """데모 org 의 `사업명` 배열. 🔴 **샘플 5건에서 «뽑는다» — 손으로 적지 않는다.**

    샘플 문안은 `mock_data.목_계획` 이 출처인데 그 5건의 사업이 4종으로 섞여 있다
    (초기창업·예비창업·창업도약·TIPS). 여기에 4종을 다시 «적어» 두면 둘이 갈리고,
    갈린 다음엔 어느 쪽이 맞는지 아무도 모른다 — `_샘플계획()` 과 같은 이유다.

    ■ 왜 1종으로 안 두나 (중앙 결정 2026-09-03)
      · 실제 `tenant.orgs` 가 그 모양이다 — 한 기관이 여러 사업을 운영하는 게 정상이다
      · 5건의 사업명을 하나로 «통일» 하면 「TIPS 멘토링 자문료」가 예비창업 계획이 된다.
        그건 문안을 지어내는 것이다
      · 온보딩의 «사업 선택» 화면이 산다. 고를 게 하나뿐이면 시연에서 죽은 화면이다

    🔴 **이건 «시연 화면이 안 비게» 하는 결정이지 판정 결정이 아니다.**
       판정 정확도는 이걸로 안 좋아진다. 두 축을 섞어 읽지 마라.
    """
...
def _낡은데모정리() -> None:
    ...
    ⚠️ **배포 전에 「이미 쌓인 `[데모]` org 가 몇 건인가」를 한 번 세라.**
       이 수정 «전» 에 비특권 롤로 돌던 동안에는 조용히 0건씩 지나갔으므로 그때 만들어진
       것들이 그대로 남아 있을 수 있다. 실측으로 갈린다:

           쌓인 것이 TTL 지남   → 첫 요청이 스스로 자리를 비운다 (200)
           쌓인 것이 아직 살아  → 429. 손으로 비우거나 TTL 을 기다려야 한다

       🔴 「생성이 계속 503 이었으니 애초에 안 쌓였을 것」은 **추론이고, 조건이 붙는다** —
          그 앞에 superuser DSN 으로 돌던 구간이 있으면 그때 것은 남는다. 세야 안다.
          (`SELECT count(*) FROM tenant.orgs WHERE 기관명 LIKE '[데모] %'` — 읽기 한 줄)
    """
    낡은 = _질의(
