...
def _GPU경로() -> bool:
    """지금 요청이 «자체 GPU(vLLM)» 로 가는가.

    🔴 2026-09-07 — GPU 워치독(게이트·기동)은 vLLM 경로에서만 의미가 있다. Qwen(외부
       API)은 팟이 꺼져 있든 말든 상관이 없는데, 워치독 호출부 세 곳이 백엔드를 안 보고
       무조건 돌아 ① 판정이 판단불가로 막히고 ② 꺼둔 팟을 도로 깨우려 든다.
       ②는 오너가 「재기동 금지」로 못박은 팟이라 더 위험하다 — 하루 기동캡(8회)을
       헛되이 깎고, 시작이 성공하면 과금이 다시 붙는다. (③ 발견 ai-3f 실측:
       `/api/normalize` 1건에 `오늘_깨움` 0→1)
       ⇒ 「환경변수를 줬다」 ≠ 「그 코드가 불린다」.
    """
    from llm_qwen import 스위치_적용   # noqa: PLC0415 — 지연 import (llm_qwen 상단 docstring)
    return 스위치_적용() == "vllm"
...
def _폼_비목후보(품목: str | None, 용도: str, 금액: float | None,
    ...
    try:
        ...
        프롬프트 = (
            ...
            "목록 안에서만 고르라.\n"
            # 🔴 2026-09-07 — 「비우거나」를 뺐다. 뒷문장은 «둘을 나란히» 의 이유만
            #    설명하는데 앞에서 「비우거나」를 먼저 제시해, 모델이 쉬운 쪽(빈 배열)을
            #    골랐다(실측: 「LG그램 노트북 180만원」→ `[]`). 빈 배열이면 화면에서
            #    AI 추천 배지가 통째로 사라져 사용자가 «아무것도» 못 받는다.
            #    「모르겠다」는 후보 둘을 낮은 신뢰도로 주는 것이지 비우는 게 아니다.
            ...
            + (f"사업명: {사업명}\n" if 사업명 else "")
            + f"\n비목 목록: {', '.join(비목_ENUM)}"
            # 🔴 2026-09-07 — `corpus.item_alias`(300행 · 「노트북→기계장치」·「랩탑」·
            #    「태블릿PC」 등)를 여기 실었다. 자연어 경로(`normalize_run`)는 이미
            #    싣고 있었는데 **폼 경로만 빠져 있었다** — 화면에서 폼으로 넣으면
            #    모델이 맨손으로 찍는다. 실측: 「LG그램 노트북」이 비목후보 «빈 배열»
            #    이었고, 힌트를 실으면 기계장치 0.95 로 잡힌다.
            #    ⇒ 「DB 에 있다」 ≠ 「프롬프트에 실린다」 — 같은 표에서 두 번째다.
            + _별칭_힌트(f"{품목 or ''} {용도 or ''}"))
        # 🔴 사고=False — 비목후보 추출에 사고가 필요 없다(위 normalize_run 주석 참고)
...
def _실_정규화(body: 정규화요청) -> dict:
    # 🔴 2026-09-07 레인 Q — `_실_판정()`에만 스위치_적용()을 걸면, 서버 기동 후
    #    `/api/normalize`가 판정보다 먼저 오는(화면 3→4 정상 순서) 첫 요청은 여전히
...
def _요약_S번호_풀기(요약: str, 인용들: list) -> str:
    ...
    🔴 2026-09-06 production 실화면 — 「AI 해설」 자리에 **`S03`** 이 그대로 나갔다.
       S번호는 프롬프트 안에서만 쓰는 «내부 인용 앵커» 다. `llm_schema.py:230` 이
       이미 같은 사고를 경고해 뒀다 — 「화면에 "지침은 「S03」 이 정하고 있습니다" 처럼
       내부 번호가 그대로 나갈 뻔했다」. 그때 «전제» 는 고쳤는데 «요약» 은 안 고쳤다.

    🔴 인용을 «생성» 하지 않는다 — 이미 검증·강등기가 DB 원문으로 채워 둔 `인용목록`
       에서 «그 S번호가 가리키는 조» 를 꺼내 이름만 바꿔 넣는다. 매칭이 안 되는
       S번호는 «지우지 않고 그대로» 둔다 — 조용히 없애면 근거가 사라진 걸 아무도 못 본다.
    """
...
def _요약_나열이면_대체(요약: str, 판정: str, 룰문장: str = "") -> str:
    """🔴 2026-09-07(중앙 ai-33, 오너 지시) — 실화면 「AI 해설」에

           `제38조 외주용역비 ③, 제38조 외주용역비 ④, 제45조 광고선전비 ⑦`

       가 나갔다. S번호를 조문으로 «푸는» 것까지는 됐는데, 애초에 모델이
       요약을 문장이 아니라 «인용 앵커 나열» 로 채운 것이 원인이다.
       run 197 실측 — 12건 중 5건이 이 모양이었고, 오답 3건이 «전부» 그 안에 있었다.

       근본 처방은 B0 프롬프트(요약 문안 규칙)와 스키마(minLength 20)에 넣었다.
       여기는 그래도 새어 나올 때의 «안전망» 이다. 지어내지 않는다 —
       판정에서 오는 사실만 쓰고, 근거는 인용란이 따로 나른다.
    """
    if not 요약:
        return 요약
...
# 🔴 2026-09-07 — 값 쪽이 «필요F필드»(대문자 F)였다. 실제 DB 칼럼은 «필요f필드»(소문자).
#    Postgres 는 따옴표 식별자가 대소문자를 구분해 `SELECT "필요F필드"` 가 UndefinedColumn
#    으로 죽는다. 그런데 `_심층질문마스터()` 의 `_질의()` 가 예외를 삼켜 «항상 빈 리스트»를
#    돌려줬다 — 그 함수 docstring 이 "컬럼이 아직 없으면 빈 리스트를 준다(정상)" 이라
#    적혀 있어서, «오타로 인한 실패»가 «정상 동작»과 같은 모양으로 숨었다.
#    ⇒ 심층질문이 화면에 «한 번도» 안 실린 원인이다. 프론트는 그때마다 하드코딩
#      폴백을 띄웠고, 사용자는 그게 서버 답인 줄 알았다. (발견 ai-3f, 실측 재현 확인)
...
def _실_판정(body: 판정요청) -> dict:
    ...
    return {
        "판정": r.get("판정", "판단불가"),
        # 🔴 S번호를 사람이 읽는 조문 표기로 바꿔서 내보낸다 (2026-09-06).
        #    화면 「AI 해설」에 `S03` 이 그대로 나가고 있었다.
        "요약": _요약_나열이면_대체(
            _요약_S번호_풀기(r.get("요약", ""),
        ...
        "버전스탬프": r.get("버전스탬프"),
        "참조사슬": r.get("참조사슬", []),
        # 🔴 2026-09-07 — `증빙목록` 을 여기서 «떨구고» 있었다. 이 dict 가 키를 «명시
        #    나열» 하는 구조라, 오케가 만든 것(`orchestrate.증빙_발급처()` — 인건비만
        #    11종 · `corpus.rules.증빙` × `corpus.evidence_sources.발급처` 조인)이
        #    여기서 통째로 사라졌다. 저장부(`persist.판정_저장`)도 화면도 못 봤고,
        #    그래서 「3. 결제 후 필요 증빙」이 «항상» 비어 있었다.
        #    ⇒ 「서버가 만든다」 ≠ 「화면이 쓴다」 — 계약 dict 가 중간에서 자른 자리다.
        "증빙목록": r.get("증빙목록") or [],
