# -*- coding: utf-8 -*-
"""「써도돼요」 백엔드 — FastAPI 앱과 판정·정규화 엔드포인트.
...
엔드포인트
    ...
    GET  /api/vocab                   비목 enum 10종
    GET  /api/programs                사업 8종 + 별칭
    POST /api/normalize   SSE         화면 3 → 4
    POST /api/judge       SSE         화면 4 → 5 / 6 / 7
    GET  /api/profile   PUT           F 프로필
    GET  /admin/cost                  비용 가드 상태
    GET  /admin/gate                  f_axis 차단 로그
    GET  /admin/queue                 재검수 큐
    POST /admin/warmup                워밍업

현물은 없다. `f1` 은 `{정부지원_현금, 자기부담_현금}` + 협약기간이고 `f3` 에 `형태` 가 없다.
PMS 원문·실명은 서버로 오지 않는다. 브라우저에서 파싱해 확인된 숫자만 받는다.
...
# 공용 정의는 `server/_common.py`, 계약은 `server/models.py` 가 기준이다. 여기서 다시 정의하지 않는다.
...
def _GPU경로() -> bool:
    """지금 요청이 자체 GPU(vLLM) 로 가는가. GPU 워치독은 vLLM 경로에서만 부른다."""
    from llm_qwen import 스위치_적용   # noqa: PLC0415 — 지연 import
    return 스위치_적용() == "vllm"
...
class 비용가드:
    """GPU 호출 비용 가드 — 일일 총 호출 캡, IP 시간당 제한, 동일 질문 캐시, 개방 창.

    캐시 열쇠는 해시다. 원문·F5 값을 메모리에 들고 있지 않는다.
    org_id 가 열쇠에 들어간다. 안 넣으면 A 기관의 판정이 B 기관에 나간다.
    """
...
# SSE 하트비트 주기(초). 실 판정이 블로킹하는 동안 프록시 idle timeout 에 끊기지 않게 한다.
_하트비트_주기초 = _int환경("SUDDOE_JUDGE_HEARTBEAT_SEC", 12)


def _ip(req: Request) -> str:
    # X-Forwarded-For 첫 홉을 쓰고, 없으면 소켓 주소를 쓴다
...
_목_정규화 = {
    ...
    # 목 모드는 check_items 를 안 읽는다. 빈 리스트로 실 모드와 키를 맞춘다
    "심층질문": [],
}

# 판정 4종 목. 프론트는 ?목=조건부|불가|판단불가 로 갈아끼워 네 화면을 다 만든다.
_목_판정: dict[str, dict] = {
    "가능": {
        "판정": "가능",
        "요약": "1인 1대 한도 내에서 구매 가능합니다.",
        # `code` 는 `corpus.check_items` 의 실제 마스터 값만 쓴다. 없으면 `code: None` 에 `구분` 을 안 붙인다.
        # SSE 는 `code`(영문), 저장 할일(`models.할일`)은 `코드`(한글) — 앞은 프론트 계약, 뒤는 DB 컬럼 계약.
        "해야할일": [
    ...
    "조건부": {
        ...
        "해야할일": [
            # 마스터에 기계장치 사전승인 code 가 없다. 지어내지 않고 `구분` 도 안 붙인다.
            {"code": None,
    ...
    # 판단불가는 에러가 아니라 화면 9(문의 초안)로 이어지는 안내다.
    "판단불가": {
        "판정": "판단불가",
...
def _기동시_워밍업() -> None:
    """기동 단계에서 동기로 모델을 데운다 — 첫 사용자가 콜드 로드를 안 문다.

    Cloud Run 은 요청을 처리하는 동안만 CPU 를 줘서 백그라운드 스레드로는 못 데운다.
    목 모드에서는 안 돈다. `SUDDOE_WARMUP_ON_START=0` 으로 끌 수 있다.
    """
...
# ── 인증·테넌트 귀속 ──────────────────────────────────────────────────
# CORS 보다 먼저 add 한다 — add_middleware 는 나중 것이 바깥이라 이 순서라야 401·503
# 응답에도 Access-Control-Allow-Origin 이 붙는다(반대면 브라우저가 인증 거부를 CORS
# 차단으로 오인한다).
# 목 모드에선 붙이지 않는다 — JWKS·DB 가 없어 Bearer 검증이 503/403 이 된다. 목은
# ?org_id= 자기신고로 돈다.
if not MOCK:
...
app.add_middleware(
    ...
    # DELETE 가 없으면 브라우저 사전확인(OPTIONS)이 400 으로 막혀 실제 DELETE 요청이
    # 서버에 닿지 못한다.
...
# ── 라우터 ──────────────────────────────────────────────────────────
# 계통별로 갈린다: 지출계획=routes_plans · 할일=routes_tasks · L3=routes_l3 + 아래 normalize.
#    남의 라우터 파일은 고치지 않는다.
...
app.include_router(routes_admin_ingest.router)  # /admin/ingest·parse_report

# IDLE_MIN=0 이면 스레드를 만들지 않는다. 목 모드 가드는 gpu_watchdog 안에 있다 —
# 없으면 목 서버가 30분 뒤 실 팟에 stop 을 쏜다.
_워치독.시작_루프()
...
# ── 사업명 정본화 ──────────────────────────────────────────────────────
# 진입점(사업명 쿼리파라미터·body.사업명)에서만 검사한다. 다운스트림은 파라미터화
# 쿼리라 모르는 표기는 조용한 0행으로 실패하기 때문이다.
# 모르는 표기는 422 로 거부한다 — 조용히 통과시키면 `비목계통()` 이 「창업」으로
# 잘못 떨어져 근거 있는 척하는 오답이 나간다.
# 정규화는 연도 접두사·공백·가운뎃점(・→·) 흔들림만 흡수한다. 이름 자체가 다른
# 표기는 정규화가 아니라 `corpus.programs.별칭` 에 별칭으로 등록한다.
...
def _사업명_표(강제새로: bool = False) -> dict[str, str]:
    ...
    TTL 캐시한다 — `_질의()` 는 호출마다 새 psycopg 접속을 열어 지연이 크다.
    빈 표는 캐시하지 않는다 — DB 가 깜빡인 순간을 오래 물고 있지 않게 한다.
    """
    global _사업명_캐시
...
def _사업명_정본(값: str | None) -> str | None:
    """모르는 표기면 422 로 명시적으로 거부한다. 조용히 통과시키지 않는다.

    조건은 「표가 비었나」가 아니라 「MOCK 인가」로 판단한다 — `_질의()` 는 DB 오류도
    빈 표로 돌려주므로, 표가 비었다는 사실만으로는 목 모드와 DB 장애를 구분할 수 없다.
    """
    if not 값:
        return 값
    if MOCK:
    ...
    표 = _사업명_표()
    if not 표:
        # 라이브인데 사업 목록을 못 읽었다 — 모르는 사업이 아니라 판단할 수 없음이다.
...
# ── 비목 정본화 ──────────────────────────────────────────────────────
# `corpus.item_vocab.별칭` 에 프론트 표기가 이미 있다 — `확정비목 not in 비목_ENUM`
# 검사가 정본만 보고 422 를 던졌을 뿐이다.
# 정본 표기가 원문과 다른 경우(예: 기계장치/기계장치비, 무형자산 취득비의 공백)는
# 별칭으로 흡수한다. 공백을 접은 키도 2차 그물로 함께 담아 원문 표기를 받는다.
...
def _비목_표() -> dict[str, str]:
    """키 → 정본. 정본·별칭과 공백을 접은 키를 함께 담는다.

    사업명 표와 같은 이유로 캐시한다. 빈 표는 캐시하지 않는다.
    """
    global _비목_캐시
...
# 한해 DB(`tenant.judge_cache`)로 옮긴다. 키 설계·설정_해시 무효화 축의 이유는
# `db/init/13_judge_cache.sql` 머리말이 정본이다.
#
# 게스트(org 없음)는 그대로 인메모리다 — `tenant.*` RLS 정책이 `org_id = current_org()`
# 라 NULL 은 어느 쪽이든 통과하지 못한다. 게스트 캐시는 재배포로 날아가도 안전하다
# (다음 요청이 다시 계산).
...
def _설정_해시(강제새로: bool = False) -> str:
    ...
    개수(행수·verified 수)만으로는 칸 단위 UPDATE(행수는 그대로 두고 내용만 고치는
    수정)를 못 잡는다 — 판정에 쓰이는 컬럼 전부를 rule_id 순으로 이어붙여 md5 한다.
    TTL 로 캐시한다. DB 를 못 읽으면 `"unknown"` 을 주고 캐시하지 않는다.
    """
    global _설정_해시_캐시
...
def vocab(사업명: str | None = None) -> dict:
    """비목 enum 10종. 창업활동비는 예비창업패키지에만 있다."""
...
def _우선순위규칙맵() -> dict[str, list[우선순위규칙]]:
    ...
    org 경계가 없는 공통 참조 데이터다(사업명으로만 갈린다). `verified` 만 거른다.
    """
...
def programs() -> 사업목록응답:
    """사업 목록. 게스트에게 화면 3 에서 사업을 묻기 위한 재료.

    룰 조회 키가 `사업 × 비목` 이라 사업을 모르면 한도를 못 고른다 — 게스트에게도
    물어야 한다.
    `우선순위규칙` 필드는 8사업 공통 관리기준(L1/L2/L3 우선순위) 표다. 새 엔드포인트로
    안 뺀 이유: `/api/programs` 와 같은 생명주기·캐시 특성이라 왕복만 늘어난다.
    """
...
def normalize(req: Request, body: 정규화요청):
    ...
    # 캐시 열쇠를 만들기 전에 정본화한다 — 안 그러면 표기가 다른 같은 사업이 다른
    # 열쇠로 갈려 같은 질문이 두 번 돈다.
    ...
    def gen():
        ...
        if 캐시 is not None:
            ...
            return
        # 첫 진행 이벤트 뒤에 둔다 — 앞에 두면 팟 상태 조회(타임아웃 15초)가 첫
        # 바이트를 막아 화면이 최대 15초 빈다.
        if not MOCK and body.질문 and body.질문.strip() and _GPU경로():
            for 진 in _워치독.기동_진행():
                yield _sse("진행", 진)
        try:
...
def _합성_질문(body: 정규화요청) -> str:
    ...
    이 문장을 다시 LLM 입력으로 쓰지 않는다(필드→문장→필드 왕복은 정보를 잃는다).
    저장·검색·표시 전용이다.
    """
...
def _주체org(req: Request) -> str | None:
    ...
    `auth.OrgId주입` 은 쿼리스트링만 갈아끼운다 — 요청 본문의 org_id 는 안 닿아서
    라우터가 `scope["suddoe_주체"]` 를 직접 읽는다.
    """
...
def judge(req: Request, body: 판정요청, 목: str | None = None):
    ...
    판단불가는 에러가 아니라 정상 경로다. `저장` 은 부수 효과라 실패해도 스트림은
    항상 `완료` 까지 간다.
    실 판정 대기 중에는 `SUDDOE_JUDGE_HEARTBEAT_SEC`(기본 12초)마다 SSE 주석
    (`: keep-alive`)이 낀다 — idle timeout 으로 스트림이 끊기는 걸 막는다.
    """
    ok, 사유 = 가드.통과(_ip(req))
    if not ok:
        raise HTTPException(429, 사유)
    # 422 앞에서 별칭을 푼다 — 프론트 표기가 정본과 다른 것들이 있다.
    body.확정비목 = _비목_정본(body.확정비목)
    if body.확정비목 and body.확정비목 not in 비목_ENUM:
        raise HTTPException(422, f"비목은 enum 10종 뿐입니다: {비목_ENUM}")
    # 비목과 같은 자리에서 닫는다 — 통과하면 `비목계통()` 이 모르는 표기를 「창업」으로 삼킨다.
    body.사업명 = _사업명_정본(body.사업명)
    # 토큰이 있으면 본문의 org_id 를 버린다 — 안 하면 org_id=None 으로 돌아 L3 규정이
    # 안 실리고, 비용가드 열쇠도 org 없이 잡혀 A 기관 판정이 B 기관에 나간다.
    ...
    def gen():
        ...
        if 캐시 is not None:
            ...
            # Qwen 경로면 아예 안 탄다 — 꺼둔 팟을 깨우려 들면 안 된다(`_GPU경로`).
            if _GPU경로():
                for 진 in _워치독.기동_진행():
                    yield _sse("진행", 진)
            # 별도 스레드에서 돌리고 짧은 주기로 폴링한다 — 대기 중에는 SSE 주석
            # (`: keep-alive`)만 흘린다. 새 이벤트 이름은 만들지 않는다 — 이벤트
            # 목록·순서는 계약이다. 무한정 기다리되 하트비트가 판정보다 먼저 포기하지 않는다.
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
                ...
                while True:
                    try:
                        ...
                        # 실패해도 4-way 밖으로 나가지 않는다 — 판정 어휘는 닫혀 있다.
                        # `_오류` 는 계약 밖 키라 SSE 로는 안 내보내고, 서버 로그에 남긴다.
        ...
        # `증빙목록` 은 저장용 내부 키다 — `persist.판정_저장()` 이 이걸 읽어 `plan_tasks`
        # 에 구분='결제후' 로 심는다. 프론트 계약(`결과` 이벤트)에는 안 싣는다 — 목과
        # 실경로의 키집합이 갈린다. 아래 `저장` 이벤트가 `out` 을 그대로 써야 해서
        # 복사본에서만 뺀다.
...
def profile_put(body: 프로필 = Body(...), org_id: str | None = None) -> dict:
    """PMS 원문·실명·과제번호는 받지 않는다. 스키마에 그 자리가 없다."""
    if MOCK or not org_id:
...
def admin_queue(x_admin_token: str | None = Header(default=None),
    ...
    `corpus.recheck_queue` 를 그대로 읽는다.
    읽기 전용이다 — 큐를 승인해 `corpus.rules` 로 올리는 경로는 여기 없다. 그건
    사람이 검수하는 일이라, API 로 열면 검수 없이 룰이 바뀔 길이 생긴다.
    """
    _관리자(x_admin_token)
    where, 인자 = ['"상태" = %s'], [상태]
...
def admin_gpu_pod(pod_id: str = Body(..., embed=True),
    ...
    적재 경로만이다 — 판정 로직(`gpu_watchdog.py`·`adapter.py`)은 손대지 않는다.
    vllm_url 은 RunPod 프록시 규칙(`{pod_id}-8000.proxy.runpod.net`, 고정 포트 8000)
    으로 pod_id 하나에서 유도한다.
    `상태` 컬럼은 건드리지 않는다 — 가동 여부 판정은 `/api/gpu/wake` 몫이다. 이
    엔드포인트는 어느 팟을 볼지만 알려준다.
    """
    _관리자(x_admin_token)
    ...
    # `예외전파=True` — `_실행()` 기본값(삼킴)이면 rowcount=-1 뿐이라 무엇이 죽었는지
    # 안 보인다. 여기서 켜서 진짜 예외를 응답에 싣는다.
    try:
        n = _실행(
    ...
    # adapter.vllm_url() 은 30초 TTL 캐시다 — 강제갱신 안 하면 최대 30초간 옛 주소를 쓴다
    try:
...
def admin_warmup(x_admin_token: str | None = Header(default=None)) -> dict:
    ...
    콜드 스타트가 두 군데다 — 임베딩 모델 로딩과 vLLM 의 `guided_json` 첫 호출
    스키마 컴파일. 둘 다 미리 태운다.
    """
    _관리자(x_admin_token)
    # `retrieve.질문벡터` 는 실재하지 않는다 — 실제 공개 표면은 `워밍업()`·`임베딩()`
    # 이다. 파이프라인의 공개 표면만 부른다. 검색+비목확정 둘 다 `_워밍업_전부()`
    # 한 곳에서 데운다.
    결과 = _워밍업_전부()
    t = time.time()
    try:
...
def _검증오류(req: Request, exc: RequestValidationError):
    ...
    pydantic 기본 형식은 `{"detail":[{...}]}` 이라 계약상 모든 4xx·5xx 는
    `{오류, 상태}` 로 통일한다(`models.오류응답`).
    """
    첫 = (exc.errors() or [{}])[0]
...
def _폼_비목후보(품목: str | None, 용도: str, 금액: float | None,
    ...
    응답 모양은 자연어 경로와 동일하다 — `[{"비목", "신뢰도"}]`. `호출자리1_스키마()`
    에서 그대로 떼어 써서 enum 10종이 guided_json 으로 닫힌다.
    ...
    try:
        ...
        프롬프트 = (
            ...
            "목록 안에서만 고르라.\n"
            # 「비우거나」라는 표현은 쓰지 않는다 — 모델이 쉬운 쪽(빈 배열)을 고르기
            # 쉬워진다. 모르겠으면 후보 둘을 낮은 신뢰도로 준다.
            "확신이 없으면 **둘을 나란히 낮은 신뢰도로** 둔다. 억지로 하나를 고르지 마라 — "
            "갈리면 코드가 두 경로를 모두 판정한다.\n"
            "판정을 하지 마라. 가능·불가를 여기서 말하지 않는다.\n\n"
            f"품목: {품목}\n금액: {금액문}\n용도: {용도 or '(적지 않음)'}\n"
            + (f"사업명: {사업명}\n" if 사업명 else "")
            + f"\n비목 목록: {', '.join(비목_ENUM)}"
            # `corpus.item_alias`(노트북→기계장치 등) 힌트를 여기서도 싣는다 — 자연어
            # 경로는 이미 싣고 있었는데 폼 경로만 빠져 있었다.
            + _별칭_힌트(f"{품목 or ''} {용도 or ''}"))
        # 사고=False — 비목후보 추출에 사고가 필요 없다
        출력, _메타 = llm_호출(프롬프트, 스키마, 사고=False, 모델=MODEL_1 or None,
                          최대토큰=200, 타임아웃=60)
        후보 = (출력 or {}).get("비목후보") if isinstance(출력, dict) else None
        # enum 밖을 먼저 버리고 그다음 3건으로 자른다 — 순서를 바꾸면 앞자리 enum
        # 밖 항목이 멀쩡한 후보를 밀어낸다(스키마 maxItems 도 3이다).
        return [{"비목": c["비목"], "신뢰도": float(c.get("신뢰도") or 0.0)}
                for c in (후보 or [])
                if isinstance(c, dict) and c.get("비목") in 비목_ENUM][:3]
    except NameError:
        # 배선 실수(import 누락 등)는 따로 잡는다 — 아래 넓은 except 가 똑같이 빈
        # 배열로 삼키면 「LLM 이 못 골랐다」와 「코드가 안 돈다」가 구분이 안 된다.
...
def _실_정규화(body: 정규화요청) -> dict:
    # `_실_판정()`에만 스위치_적용()을 걸면 `/api/normalize` 가 먼저 오는 첫 요청은
    # 여전히 vLLM 원본을 탄다. 두 하위 경로가 모두 이 함수를 거치므로 여기서 한 번
    # 부르면 둘 다 커버된다.
    from llm_qwen import 스위치_적용
    백엔드 = 스위치_적용()
    if body.질문 and body.질문.strip():
        # GPU 게이트는 vLLM(자체 GPU) 경로에서만 의미가 있다 — Qwen(외부 API) 경로는
        # 팟 상태와 무관하다.
        if 백엔드 == "vllm":
        ...
        # 폼 경로 — 품목·금액·용도는 이미 구조화돼 왔다. 비목후보만 LLM 으로 채운다.
    ...
    # 자연어 경로는 지급수수료류에서 하위항목을 뽑을 수 있다 — 폼 경로는 못 뽑으니
    # 키만 None 으로 살린다. 경로마다 키 유무가 다르면 프론트 분기가 갈린다.
    out.setdefault("하위항목", None)
    # 내부 키는 여기서 벗긴다 — `누락필드`·`_출처` 는 `normalize_run.정규화` 산출물이지
    # API 계약이 아니다. 필요해지면 계약에 먼저 올리고 세 경로에 같이 싣는다.
    out.pop("누락필드", None)
    out.pop("_출처", None)
    # check_items 기반 심층질문. LLM 호출 0 (배선일 뿐).
    out["심층질문"] = _심층질문_선별(body.사업명, out.get("비목후보"))
    return out


def _요약_S번호_풀기(요약: str, 인용들: list) -> str:
    """`요약` 안의 S번호를 사람이 읽는 조문 표기로 바꾼다.

    S번호는 프롬프트 안에서만 쓰는 내부 인용 앵커다 — 화면에 그대로 나가면 안 된다.
    인용을 생성하지 않는다 — `인용목록` 에서 그 S번호가 가리키는 조를 꺼내 이름만
    바꿔 넣는다. 매칭이 안 되는 S번호는 지우지 않고 그대로 둔다.
    """
    if not 요약 or not 인용들:
        return 요약
    표: dict[str, str] = {}
    for c in 인용들:
        if not isinstance(c, dict):
            continue
        s번호 = c.get("s번호") or c.get("S번호")
        if not s번호:
            continue
        조 = " ".join(str(x) for x in (c.get("조번호"), c.get("조제목")) if x)
        항 = c.get("항호")
        이름 = (조 + (f" {항}" if 항 else "")).strip()
        if 이름:
            표[str(s번호)] = 이름
    if not 표:
        return 요약
    # 🔴 정규식 대신 «긴 S번호부터» 단순 치환한다 — S3 가 S30 을 깨뜨리지 않게 길이 역순.
    for _s in sorted(표, key=len, reverse=True):
        요약 = 요약.replace(_s, 표[_s])
    return 요약


# 「요약이 문장이 아니라 인용 나열뿐」인지 가른다.
# 🔴 «앵커를 품었다» 로만 가르면 안 된다 — "제38조에 따라 …필요합니다." 같은
#    «정상 문장» 까지 잡아먹는다(실측 오탐). 그래서 두 조건을 «둘 다» 본다:
#      ① 조번호·S번호 앵커가 들어 있고            ②  «문장으로 끝나지 않는다»
#    나열은 종결어미가 없다. 반대로 앵커가 아예 없으면(예: "해외 출장 시 사전승인 필요")
#    짧아도 사람이 쓴 요약이므로 건드리지 않는다.
_앵커있음 = re.compile(r"제\d+조|S\d{1,2}")
_문장끝 = re.compile(r"(?:습니다|합니다|됩니다|입니다|세요|해요|어요|죠)[.!?]?\s*$|[.!?]\s*$")


def _요약_나열이면_대체(요약: str, 판정: str, 룰문장: str = "") -> str:
    """요약이 인용 앵커 나열(조문·S번호)뿐이고 문장이 아니면 정형 문구로 바꾼다.

    안전망이다 — 지어내지 않는다. 판정에서 오는 사실만 쓰고, 근거는 인용란이 나른다.
    """
    if not 요약:
        return 요약
    if not _앵커있음.search(요약) or _문장끝.search(요약):
        return 요약
    기본 = {
        "가능": "규정상 집행할 수 있습니다. 아래 근거 조항을 확인해 주세요.",
        "조건부": "조건을 채우면 집행할 수 있습니다. 아래 확인 항목을 먼저 챙겨 주세요.",
        "불가": "규정상 집행할 수 없습니다. 아래 근거 조항을 확인해 주세요.",
        "판단불가": "주어진 근거만으로는 판단할 수 없습니다. 담당자에게 확인해 주세요.",
    }
    return 기본.get(판정 or "", "아래 근거 조항과 확인 항목을 살펴봐 주세요.")
    return 요약


def _할일_중복제거(할일: list) -> list:
    """조립 응답에 반복 생성되는 같은 항목을 없앤다.

    순서를 보존한다 — 재정렬하면 재현성이 깨진다.
    개수를 줄이는 것이 목적이 아니다 — 같은 code 에 다른 항목은 정상이라 `code`
    만이 아니라 세 필드 전부(code·항목·설명)를 키로 쓴다.
    """
    본, 본것 = [], set()
    for h in 할일 or []:
...
def _체크마스터() -> dict[str, tuple[str, str]]:
    ...
    표를 통째로 캐시한다 — 비용이 행 수가 아니라 접속 수에 붙는다. 빈 표는
    캐시하지 않는다.
    """
    global _체크마스터_캐시
...
# `corpus.check_items` 컬럼명 매핑. «유형」은 `할일.유형` 과 겹치지 않게 «질문유형」으로
# 잡았다. Postgres 따옴표 식별자는 대소문자를 구분한다 — 컬럼명 오타(대소문자 포함)는
# `_질의()` 가 예외를 삼켜 조용히 빈 리스트가 된다.
_심층질문_컬럼 = {"질문문": "질문문", "유형": "질문유형", "선택지": "선택지", "필요F필드": "필요f필드"}


def _심층질문마스터() -> list[dict]:
    """`corpus.check_items` 에서 심층질문 후보 전부를 읽는다. `_체크마스터()` 와
    같은 이유로 통째로 캐시한다.

    컬럼이 아직 없으면 `_질의()` 가 예외를 삼켜 빈 리스트를 준다 — 정상 동작이다.
    근거가 없는 행은 절대 안 낸다.
    """
    global _심층질문_캐시
...
def _심층질문_선별(사업명: str | None, 비목후보: list[dict] | None) -> list[dict]:
    ...
    못 거르면(사업명 미확정) 전부 준다 — 서버가 추측으로 줄이면 「해당 없음」이
    「아직 모름」을 삼킨다.
    """
    표 = _심층질문마스터()
    if not 표:
...
def _할일_보강(할일: list) -> list:
    """`code` 로 마스터를 봐서 `구분`·`유형` 을 얹는다. LLM 호출 증가 0.

    LLM 이 내지 않는다 — 조립 스키마의 `해야할일` 은 `{code, 항목, 설명}` 뿐이고,
    `구분`·`유형` 의 정본은 `corpus.check_items` 컬럼이다.
    `check_items.구분` 은 2종(결제전·결제후)이고 `models.할일.구분` 3종의 부분집합이다
    — 「집행」은 사용자 직접 추가와 캘린더에서 온다.
    마스터에 없는 code 면 붙이지 않는다. DB 가 죽어도 판정을 죽이지 않는다 —
    `구분` 없이 그대로 나간다.
    """
    표 = _체크마스터()
    if not 표:
...
def _사용자F값_조립(답변: list) -> dict | None:
    """심층질문 답 → `orchestrate.판정(사용자F값=...)` 입력.

    컬럼명 그대로 담는다 — dotted-path 변환은 `f값_경로키()` 안에서 한 번만 일어난다.
    목록 밖(마스터에 없는) code 는 조용히 버린다.
    """
    if not 답변:
        return None
...
def _실_판정(body: 판정요청) -> dict:
    # SUDDOE_LLM=vllm|qwen 스위치. orchestrate 를 import 하기 직전이 유일한 안전지점이다.
    from llm_qwen import 스위치_적용
    백엔드 = 스위치_적용()
    # GPU 게이트는 vLLM(자체 GPU) 전용이다 — Qwen(외부 API) 경로는 팟 상태와 무관하다.
    if 백엔드 == "vllm":
        _워치독.게이트()
    import orchestrate
    # `질문원문` 은 계약 필드다 — 없다고 문장을 되짚어 다시 만들면 사용자 확정값이
    # 조용히 죽는다. `_합성_질문()` 문장을 되먹이지 않는다(필드→문장→필드는 정보를 잃는다).
    ...
    # 사용자가 화면 9 에서 확정한 비목을 엔진에 넘긴다 — `orchestrate.py` 가 후보를
    # 이 값으로 필터하고, 없으면 「갈래고정」으로 세워 그 비목으로 판정한다.
    # 이미 돈 정규화 결과를 넘긴다 — 안 넘기면 orchestrate 안에서 다시 돌아 LLM 호출이
    # 하나 늘고, 되짚은 문장으로 다시 정규화하면 사용자 확정값이 조용히 죽는다.
    # 인자 이름이 `정규화` 가 아니라 `정규화결과` 인 이유는 모듈 전역 함수를 파라미터가
    # 가리는 걸 피하기 위해서다.
    r = orchestrate.판정(질문, 사업명=body.사업명, org_id=body.org_id, plan_id=body.plan_id,
                       _비목고정=body.확정비목 or None,
                       정규화결과=body.정규화,
                       # 심층질문 답. 없으면(빈 리스트) None 이라 orchestrate 쪽에서
                       # 종전과 같은 경로를 탄다.
                       사용자F값=_사용자F값_조립(body.답변))
    전제 = r.get("전제") or r.get("전제목록") or []
    # 후보에 없는 비목을 고정하면 그 비목으로 본 판정이 나온다 — 조용히 내보내면
    # 사용자는 시스템이 동의한 걸로 읽는다. 새 SSE 이벤트 대신 `전제` 맨 앞에 얹는다.
    ...
    if body.확정비목 and 후보 and body.확정비목 not in 후보:
        # 조사는 「로」로 고정한다 — 비목 10종이 모두 모음으로 끝난다
        전제 = [{"사실": f"비목을 「{body.확정비목}」로 보고 판정했습니다 — 정규화가 제시한 "
                      f"후보({' · '.join(후보)})에 없는 비목을 직접 고르셨습니다",
                "근거조항": None, "매핑": [], "미충족시": "판단불가"}, *전제]
    # 문의초안은 LLM 을 부르지 않는다 — 이미 손에 쥔 값으로 `server/inquiry.py` 가
    # 문장을 조립한다. 오케가 자기 초안을 주면 그걸 이긴다 — 덮어쓰지 않는다.
    if not r.get("문의초안"):
        r["문의초안"] = inquiry.문의초안(
            body.정규화 or {}, r.get("판정", "판단불가"),
            r.get("인용") or r.get("인용목록") or [], 전제, body.사업명)

    # 오케 반환값 → API 계약. 이름이 다른 것만 옮긴다
    return {
        "판정": r.get("판정", "판단불가"),
        # S번호를 사람이 읽는 조문 표기로 바꿔서 내보낸다
        "요약": _요약_나열이면_대체(
            _요약_S번호_풀기(r.get("요약", ""),
                           r.get("인용") or r.get("인용목록") or []),
            r.get("판정", "")),
        "해야할일": _할일_보강(_할일_중복제거(r.get("해야할일", []))),
        # 오케는 `인용목록` 으로 돌려준다(`llm_schema.최종응답`). 목 응답은 `인용` 을
        # 직접 쓴다 — 목과 실경로 키 이름이 달라 아래처럼 폴백으로 맞춘다.
        "인용": r.get("인용") or r.get("인용목록") or [],
        "전제": 전제,
        "신뢰등급": r.get("신뢰등급"),
        "버전스탬프": r.get("버전스탬프"),
        "참조사슬": r.get("참조사슬", []),
        # `증빙목록` — 이 dict 는 키를 명시 나열하는 구조라, 안 실으면 오케가 만든 값이
        # 여기서 통째로 사라진다.
        "증빙목록": r.get("증빙목록") or [],
...
def _판정_저장_시도(body: 판정요청, out: dict, 캐시적중: bool, decision_id: int | None) -> dict:
    ...
    지연 import 다 — `server.persist` 가 없으면 앱 전체가 못 뜨는 걸 막는다.
...
def _selftest() -> int:
    ...
    # 여기까지는 전부 `?목=` 이다 — 목에서 서는 것은 계약을 만족한다는 증거이지 실경로가
    # 판다는 증거가 아니다. 값을 만드는 함수 자체를 목을 통하지 않고 직접 태운다.
    초안 = inquiry.문의초안(
    ...
    # `_목_판정` 이 `인용` 을 직접 박아 놔서 위 judge 자가검사는 목이 서는 것만 본다.
    # 오케가 `인용목록` 으로 돌려주는 걸 `_실_판정` 이 실제로 읽는지는 이 블록이 못
    # 잡는다 — 그래서 오케를 스키마 그대로 갈아끼우고 `_실_판정` 을 직접 부른다.
    ...
    def _가짜_판정(질문, **kw):
        ...
        # 기본값([]·None)이 나오므로 빈 값 = 키 이름을 못 읽었다가 된다.
