# -*- coding: utf-8 -*-
"""지출계획 — 화면 6 홈 · 화면 7 목록 · 화면 11 상세.   **[레인 A 소유]**

Phase 0 이 목(mock) 경로를 끝까지 구현해 뒀다. 프론트는 이 파일 그대로 붙을 수 있다.
🔴 **레인 A 가 할 일은 `_실_*` 세 함수를 채우는 것뿐이다.** 목 경로와 응답 모델은 건드리지 않는다.
   응답 필드가 달라지면 프론트가 깨진다 — 필드를 바꿔야 하면 조율 세션에 보고할 것.
