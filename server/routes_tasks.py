...
def _실_동기화(plan_id: int, body: 할일동기화, org_id: str | None = None) -> 할일동기화응답:
    ...
    작업 = [
        ...
        # 🔴 `매칭금지` 는 «code 를 지어 붙이지 마라» 는 뜻이다. `corpus.rules.증빙`
        #    (「4대보험가입확인서」 같은 «서류 이름») 을 `코드_매칭()` 에 태우면
        #    체크항목 「4대보험가입」(＝"4대사회보험에 가입돼 있는지 확인하세요",
        #    구분='결제전')으로 흡수돼 버린다 — 서류가 «확인 항목» 으로 둔갑하고
        #    결제후가 결제전 칸에 찍힌다(2026-09-07 실측: 증빙 11종이 통째로 사라졌다).
        #    둘은 다른 것이다. 「가입돼 있는지 확인」은 결제 전 행동이고,
        #    「가입확인서」는 결제 후 제출 서류다.
