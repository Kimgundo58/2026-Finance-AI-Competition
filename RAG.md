...
#### 4-2b. 🔴 `retrieval_scope` 는 두 곳에 있고, 검색은 **청크 쪽**을 읽는다

`documents.retrieval_scope` 와 `chunks.retrieval_scope` 가 **137건 서로 다르다.**
드리프트가 아니라 **의도된 청크 단위 강등**이다 — 진입점 문서 안의 붙임·별표·참고 계열
청크만 골라 폐포전용으로 내린 것(119건 = 87%).

```
문서 기준으로 세면   진입점 1,591청크    <- 실제보다 크다
청크 기준(=검색 필터) 진입점 1,478청크    <- 이게 실제 후보 풀이다
```

**이 사실이 어디에도 안 적혀 있어 2026-09-01 에 실제로 오독이 났다.** 후보 풀을 셀 때는
`corpus.chunks.retrieval_scope` 를 본다.

⚠️ 강등은 총계를 올리지만 **정답이 그 안에 있으면 원리적으로 못 찾는다.** 실제로
「붙임2 창업기업등 사업비 비목 해설표」가 강등돼 골든셋 10문항이 검색 불가였다(§4-2c).

#### 4-2c. 붙임2 비목 해설표 복귀 (2026-09-01)

24청크(예비창업 붙임2 4 · 초기창업 붙임2 4 · 창업중심대학 참고3 8 · 초격차 참고2 8)를
진입점으로 되돌렸다. 주관기관용은 제외했고, 나머지 폐포전용 붙임은 그대로 뒀다.
...
- **왜 강등돼 있었나** — "붙임이 top-5 의 49.1% 를 먹는데 정답은 1건" 이었다. 그러나 그
  사유의 실체는 **표 파싱 결함**이지 붙임이라는 종류가 아니다. 4단 표(비목/세목/구분/내용)를
  세로로 읽어 「창업프로그램운영비」가 `창업/프로/-/그램/운영비` 5줄로 갈린다.
  **원인을 고치는 대신 증상을 인덱스에서 치운 것**이고 그 대가로 핵심 근거표를 잃었다
- 🔴 **재범람은 실제로 관측된다 — 총계에 가려진다.** 기존 정답들의 순위가 밀렸다
  (gold 48 20→34위 · 53 29→44 · 72 21→34 · **74 는 39→53 으로 후보 50 밖 이탈**).
  top-5 에서 빠진 건 없어 지표는 순증이지만 **여유가 줄었다.** 붙임2 재파싱이 이 작업의
  마무리인 이유가 이것이다 — 파싱이 정밀해지면 넓게 걸리지 않는다
- 되돌리기는 `chunk_id` 24개를 박은 단일 UPDATE 다. 재임베딩은 필요 없다(임베딩 불변)
...
- 🔴 **`미규정위임` 은 `resolved` 가 아니다 (2026-09-01 정정).** "정하지 아니한 사항은
  상위 규범에 따른다" 는 **어느 조인지 지목하지 않아** dst 가 비어 있다. 10건이 dst NULL 인 채
  resolved 로 세어지고 있었다. 폐포 CTE 는 `dst_조번호 IS NOT NULL` 로 걸러내므로 판정 영향은
  없었으나 라벨이 거짓이었다. 지금은 `dangling` 이고, 업로드 안내에서 이건 결손이 아니라
  법령의 정상 문형이므로 **소비자가 `관계='미규정위임'` 으로 걸러 쓴다**
- ✅ **재도전 2026판이 폐포에 들어왔다** — 판본 교체 후 재빌드로 10엣지 추가(준용 7·인용 2·별표참조 1).
  제8·9·19·23·28·29·31조가 통합관리지침 제14차로 이어진다. 교체 전에는 이 사슬이 끊겨 있었다
- ⚠️ **`eval.runs` 의 코퍼스버전이 바뀐다** — run 185~189 는 `r28701` 이다.
  refs 가 44,865 인 지금과 **폐포 재료가 다르므로** 검색·판정 지표를 직접 비교하면 안 된다
