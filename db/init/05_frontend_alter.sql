...
--
-- `accounts.팀명` 은 손대지 않았다. 🔴 2026-09-01 정정 — 「테이블 자체가 없다」고
--   적어뒀던 건 **틀렸다**. `tenant.accounts` 는 존재하고 0행이다. 없는 것은 테이블이
--   아니라 **배선**이다 (API 가 하나도 안 잇혀 있다). 둘은 남은 작업량이 다르다.
--   사이드바 계정 표시는 로그인 설계가 닫힌 뒤의 일이라 미결로 남기는 결론만 같다.
