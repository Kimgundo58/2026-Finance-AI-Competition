...
# ════════════════════════════════════════════════════════════════════
# 산문 — 조율 세션(ai-14) 이 확정한 문안. 여기가 정본이다
# ════════════════════════════════════════════════════════════════════

머리말 = """# API · DB 명세 v1

> 🔴 **이 문서는 「지금 실제로 무엇이 있는가」다.** 「왜 이렇게 주는가」(계약·근거·
> 금지사항)는 `프론트_API_계약_v1.0.md` 가 정본이고, **충돌하면 계약 문서가 이긴다.**
>
...
채워졌고 무엇이 미결인지는 **`백엔드_동결상태_0901.md` 가 정본**이다. 코드를 얹기
...
- **왜 그런가** — 계약 문서 `프론트_API_계약_v1.0.md` 를 봐라. 특히 §8 「만들지 않는
  것」과 §9 「실서버로 갈아끼울 때 버그로 오해할 것」은 이 문서에 대응물이 없다.
- **판정 품질 수치** — `결과보고.md` 가 정본이다. 일치율을 인용할 때는 **다수결
...
def api절(app) -> tuple[str, int, int]:
    ...
    # 🔴 OpenAPI 가 정본이다 — 라우트 객체로 못 본 경로가 있으면 그것도 싣는다
    메서드수 = 0
