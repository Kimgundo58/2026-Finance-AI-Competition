...
실 DB 를 검증해야 할 레인 B 테스트가 목 경로로 흘렀다. 실패는 레인 B 구현의
결함이 아니라 **테스트 하네스의 결합**이다 (단독 6/6 통과가 그 증거).
...
■ 파일이 어느 모드를 원하는지 어떻게 아는가 — 두 가지, 위가 이긴다
  ...
  둘 다 없으면 목이다. 레인은 자기 테스트 파일만 고치면 되고 이 파일은 안 건드린다.

■ 모드와 무관한 테스트도 있다 — 레인 A(`test_plans.py`)는 HTTP 를 안 거치고
...
_모듈 = (
    ...
    "server.routes_tasks",
    "server.routes_l3",
    "server.persist",          # 레인 A 보고(2026-09-01) — 판정_저장() 을 목/실 전환으로 태울 때 필요
    "server.main",
)
