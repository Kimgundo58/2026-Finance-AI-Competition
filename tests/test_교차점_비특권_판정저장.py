...
🔴 **지금 `orchestrate.판정()` 에서는 이 모양이 «아직» 안 난다** (ai-b2 재현·정정).

    `전제해소()` 는 `orchestrate.py:681`, `decisions_적재()` 는 그 «뒤» 인 `_마무리()`
    안이다. 미매핑이 터지는 시점엔 아직 저장된 판정이 없다. 게다가 예외 경로는
    `_마무리(conn, cur=None, 기록=False)` 라 `if cur is not None:` 에 걸려 **commit 을
    아예 안 부른다.** 즉 **지금 안전한 건 «구조» 가 아니라 «순서» 덕이다.**

    이 파일은 그 순서가 깨지는 날을 잡는다. 발현 조건 둘:
      ① ③(미매핑 INSERT)을 `try/except` «만» 으로 감쌀 때 — 중앙이 savepoint 를
         요구한 이유가 이것이다. except 로 삼켜도 트랜잭션은 이미 abort 다
      ② 누가 `decisions`/계획 저장을 `전제해소` «앞» 으로 옮길 때
    🔴 그래서 이 회귀는 「지금 나는 버그」가 아니라 **「순서에 기대고 있다」는 사실**을
       고정한다. 순서를 바꾸는 사람이 여기서 걸린다.
...
# 🔴 xfail 을 뗐다 (2026-09-03 · #17 머지 dc9d63f). 이 테스트는 만들 때 «빨간» 상태였고,
#    savepoint 수정(91fd456)이 들어온 순간 XPASS 로 빨개져 스스로 「표시를 떼라」고 알렸다.
#    설계대로 작동한 것이다 — 지금부터는 **평범한 초록 테스트**이고, 되돌아가면 빨개진다.
