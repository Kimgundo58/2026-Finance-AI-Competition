# -*- coding: utf-8 -*-
"""계약 검증 — `프론트_API_계약_v1.0.md` 를 **집행 가능한 테스트로** 바꾼 것.

    PYTHONIOENCODING=utf-8 python -m pytest tests/test_contract.py -q

🔴 **이 파일은 계약을 «검증» 만 한다. 결함을 고치지 않는다.**
   `server/` 는 여러 계통이 함께 쓴다. 깨지면 고치기 전에 어느 계통인지부터 가른다.

■ 왜 필요한가
  계약 문서는 사람만 읽을 수 있어서, 필드를 하나 빼먹어도 아무도 모른다.
...
계약 = {
    ...
    # 🔴 `파싱품질` 2026-09-04 추가(Q5) — DB `l3_documents."파싱품질"` 을 그대로 노출한다.
    #    `warn`(조문은 뽑혔지만 품질 낮음)이 `상태` 하나로는 「완료」에 묻혀 안 보이던 걸
    #    이 필드로 프론트가 배지를 달 수 있게 한다. `조_건수` 와 같은 이유로 필수(값은 None 가능).
    "L3":          ({"doc_id", "파일명", "확장자", "상태", "조_건수", "dangling", "메시지",
                    "파싱품질"}, set()),
    "프로필":       ({"f1", "f3", "f4"}, set()),
    # SSE 생 dict — response_model 이 안 지켜 준다
    # 🔴 `하위항목` 은 2026-09-01 까지 목·폼 경로에만 없어서 «모드마다 다른 키» 였다.
    #    `_실_정규화` 의 **공통** setdefault 로 메워 세 경로가 같아졌다 →
    #    선택에서 **필수로 올린다.** 다시 빠지면 여기서 걸린다.
    # 🔴 `심층질문` A-3(2026-09-06, 레인 Y) 추가 — `_실_정규화` 가 공통 setdefault 급으로
    #    항상 싣는다(세 경로 동일, 빈 리스트도 있음). 계약 밖으로 다시 빠지면 여기서 걸린다.
    "정규화결과":    ({"품목", "금액", "금액_추정여부", "용도", "비목후보", "하위항목",
                    "결제수단", "구매명의", "신청일", "비교견적", "질문원문", "심층질문"}, set()),
    # 🔴 `받은값` 은 계약이 아니다 (2026-09-01 판정 — 디버그 잔재)
...
@pytest.mark.xfail(reason="`받은값` 은 디버그 잔재 — 계약 아님(2026-09-01 동결). "
...
def test_check_items_구분이_늘면_알린다():
    """🔴 조건이 성립하면 터지는 것을 미리 잡는 가드다 — 지금은 모순이 아니다.

    `corpus.check_items.구분` CHECK 은 **2종**(결제전·결제후)인데 `plan_tasks.구분`
    과 `할일구분_ENUM` 은 **3종**(+집행)이다. 지금은 「집행」을 사용자 추가 할일만
    쓴다는 뜻이라 정합적이다.

    그런데 마스터 표에 `구분='집행'` 행이 생기면, 판정이 그 코드를 실어 보낼 때
    `routes_tasks._유형_맵` 경로가 처음 보는 구분을 만난다 — **조용히 틀리는 종류**다.
    그날 이 테스트가 빨개진다. 빨개지면 지우지 말고 동기화 경로를 점검한 뒤 여기를
    3종으로 넓혀라. (`유형` 컬럼이 생겼을 때 하드코딩 맵이 낡은 채 남았던 것과 같은 사고를 막는다.)
    """
    행 = _질의('SELECT DISTINCT "구분" FROM corpus.check_items')
    if not 행:
        pytest.skip("corpus DB 미기동 — check_items 를 못 읽는다")
    값 = {r[0] for r in 행}
    assert 값 <= set(할일구분_ENUM), f"check_items 에 할일구분_ENUM 밖 값이 있다: {값}"
    assert "집행" not in 값, (
        "check_items 에 구분='집행' 행이 생겼다 — 판정이 이 코드를 실어 보내면 "
        "`_유형_맵`·동기화 경로가 처음 보는 구분을 만난다. 점검 후 이 가드를 넓혀라")
...
# 계약 §6. 🔴 `저장` 은 2026-09-01 계약에 넣은 판정 영속화 배선이다.
...
#    프론트가 이걸 빨간 배너로 그리면 안 된다 (2026-09-01 확인).
...
def _vlm_문서() -> set[str]:
    """규정 모음에서 실제 vlm 로 태깅된 doc_id. DB 가 없으면 빈 집합."""
...
def test_A등급_판정은_vlm_을_인용하지_않는다(목판정):
    vlm = _vlm_문서()
    if not vlm:
        pytest.skip("규정 모음 DB 미기동 — vlm 문서 목록을 못 읽는다")
...
def test_인용은_원문_문자열_필드를_따로_안_만든다():
    ...
    **코드가 규정 모음에서 떠 온 것**이라야 하고, 그 자리에 doc_id 가 반드시 따라야 한다."""
...
def test_실판정_실패시에도_계약_키만_나온다(monkeypatch):
    ...
    걷어내고 대신 `logging` 으로 옮겼다 — **지운 게 아니라 자리를 옮긴 것**이라
...
계약_보류: set[tuple[str, str]] = {
    ...
    # 🔴 2026-09-04 — 관리·스케줄러 전용이라 프론트 계약이 아니다(중앙 판단).
    #    Cloud Scheduler 가 5분 주기로 이 경로를 친다 — 사람이 브라우저로 안 부른다.
    ("POST", "/api/gpu/reap"),
}
...
def _실제_엔드포인트() -> set[tuple[str, str]]:
    ...
    **하위 라우터의 엔드포인트 9개가 통째로 «없음» 으로 잡힌다.**
    OpenAPI 스펙이 «프론트가 실제로 보는 것» 이기도 하니 그쪽을 기준으로 쓴다.
    """
...
def test_계약에_없는_api_가_늘지_않았다():
    늘어남 = ({(m, p) for m, p in _실제_엔드포인트() if p.startswith("/api")}
            - 계약_엔드포인트 - 계약_보류)
    assert not 늘어남, (f"계약 문서에 없는 /api 경로가 생겼다: {sorted(늘어남)} — "
                      f"프론트가 계약 밖 경로를 쓰기 시작하면 그게 다음 사고다")


# 🔴 2026-09-01 삭제 — `test_계약이_동결한_게스트_발급이_살아있다`
#    「계약이 `POST /api/guest` 를 동결로 박았는데 서버에 없다」로 xfail 을
#    걸어뒀고, 고칠 방향 둘 중 **②(계약 문서에서 §2-2 를 내린다)** 를 택했다.
#    §2-2 는 「미결 — 게스트끼리 서로 보인다」로 다시 썼고 §1 표의 행도 뺐다.
#    D 의 지시대로 이 테스트는 지운다 — 동결이 아닌 것을 지키는 테스트는 남기지 않는다.
#    게스트 격리를 다시 칠 때는 `test_tenant_isolation.py` 의
#    `test_게스트끼리는_지금_서로_보인다_…` 가 빨개지면서 알려준다.
