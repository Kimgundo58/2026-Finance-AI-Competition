# -*- coding: utf-8 -*-
"""E2E — 계획 생성 → 판정 저장 → 화면(목록·상세·탭·통계) 반영. **[지출계획 계통]**
...
`test_contract.py` 는 계약 표면(엔드포인트·이벤트·스키마)만 본다 — 겹치지 않는다.
...
def test_판정4way_저장_그리고_탭에_반영(정리):
    ...
    for 판정 in ("가능", "조건부", "불가", "판단불가"):
        ...
        # ⑦ 코드가 실린 해야할일의 유형이 `할일유형_ENUM` 안인지 (유형 소스를
    ...
    #
    # 🔴 **2026-09-02 — 여기가 유령실패의 자리였다.** 원래는 탭마다 GET 을 4번 하고
    #    네 결과가 «전부 같은지» 를 봤다. 그런데 `_실_통계(None)` 은 **게스트 버킷 전체**를
    #    센다 — 이 테스트가 만든 것만이 아니다. 그래서 **4번의 GET 사이에** 다른 세션이
    #    게스트 계획을 하나 만들면 뒤쪽 세 값이 통째로 달라진다.
    #    실제로 발동시켜 재현했다: 세 번째 GET 직전에 1건을 INSERT 하니
    #    `전체 0,0,1,1,1` 로 갈렸다. 단독 실행은 늘 통과하고 전체 스위트에서만 났다.
    #
    #    🔴 `test_persist` 에서 배운 것과 같다 — **좁히면 재는 시점이 같이 바뀐다.**
    #    이 assert 의 의도는 「통계가 **탭 필터에 안 묶인다**」이지 「절대값이 고정」이 아니다.
    #    그래서 절대값을 비교하지 않고, 각 탭 조회를 **「전체」 조회로 감싸서**
    #    그 사이에 DB 가 움직였는지 스스로 판별하게 한다
    #    (`test_tenant_isolation._맞대보기` 가 이미 쓰는 방식이다).
    #    ⚠️ 「앞뒤 「전체」 조회가 같으면 그 사이도 안 움직였다」로 먼저 짰는데 **그것도 틀렸다.**
    #    남이 INSERT 후 곧바로 DELETE 하면 앞=0 · 중간=1 · 뒤=0 이라 감싸기를 통과한다.
    #    실제로 훼방 스레드를 돌려 30회 중 **오탐 2건**을 봤다.
    #    → 감싸기 대신 **재시도**로 간다. 진짜로 탭에 묶여 있다면 «몇 번을 해도» 어긋나고,
    #      잡음이면 한 번은 조용한 창이 온다. 「한 번이라도 일치하면 통과」가 옳은 판정이다.
    def _통계(탭: str) -> dict:
        return client.get("/api/plans", params={"탭": 탭}).json()["통계"]
    ...
    for 탭 in 탭_판정:
        ...
        for _ in range(8):
            ...
            뒤 = _통계("전체")
            if 앞 == 뒤 == 값:
                break                      # 조용한 창에서 일치했다 — 이 탭은 통과
            마지막 = (앞, 값, 뒤)
        else:
            raise AssertionError(
                f"통계가 탭 «{탭}» 에서 8번 연속 흔들렸다 — 필터(탭) 적용 전 전체 기준이 "
                f"아니다. 마지막 관측 (전체, {탭}, 전체) = {마지막}")

    최종 = _통계("전체")
    # 🔴 `>=` 다. 다른 세션의 게스트 계획이 섞여 들어올 수 있으므로 상한은 못 건다 —
    #    이 테스트가 만든 것만으로도 최소 이만큼은 나와야 한다는 뜻이다.
    assert 최종["확인필요"] >= 2   # 조건부 1 + 판단불가 1
    assert 최종["위험"] >= 1
    assert 최종["특이사항없음"] >= 1
