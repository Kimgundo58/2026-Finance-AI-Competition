...
def _맞대보기(이름: str, api호출, sql호출, 시도: int = 8) -> None:
    """API 집계와 «같은 경계» SQL 집계를 맞댄다 — 동시 작업에 안 흔들리게.

    🔴 **2026-09-02 — 이 함수는 「감싸기」였고 그게 틀렸다.**
       전에는 `SQL → API → SQL` 로 재고 **앞뒤가 같으면 그 사이도 안 움직였다**고 보아
       `min(앞,뒤) <= api <= max(앞,뒤)` 로 판정했다. 그런데 남이 INSERT 후 **곧바로
       DELETE** 하면 앞=1 · 중간=0 · 뒤=1 이라 그 창을 그냥 통과한다.
       실측: 훼방 스레드(게스트 계획 INSERT→DELETE 반복)를 돌리며 40회 중 **오탐 4건.**

       🔴 **그리고 오탐의 메시지가 최악이었다** — 「API 가 크면 남의 것이 섞인 것이다」로
       **TENANT_LEAK 을 단정**했다. 이 프로젝트에서 격리 누수는 최상위 사고라, 없는 누수를
       있다고 보고하면 다들 격리부터 뒤진다. 실제 오탐 첫 건은 `API 0 < SQL [1,1]` 로
       **API 가 더 작았는데도** 그 메시지가 나갔다.

       → `test_e2e_flow` 에서 쓴 것과 같은 모양으로 바꾼다: **앞 == 뒤 == api** 인
         «조용한 창» 을 만날 때까지 재시도한다. 진짜 누수라면 몇 번을 해도 어긋나고,
         동시 쓰기 잡음이면 한 번은 조용한 창이 온다.
       🔴 두 SQL 이 서로 다르면 그건 **누수 판정을 내릴 수 있는 관측이 아니다.**
         그래서 실패 메시지에서 「남의 것이 섞인 것이다」 단정을 뺐다.
    """
    마지막 = None
    for _ in range(시도):
        앞 = sql호출()
    ...
    raise AssertionError(
        f"{이름}: {시도}번 연속으로 API 집계가 같은 경계 SQL 과 안 맞았다 "
        f"(SQL {앞} → API {api} → SQL {뒤}).\n"
        f"  · 두 SQL 이 같은데 API 만 다르면 → 경계가 새거나 좁은 것이다\n"
        f"  · 두 SQL 이 서로 다르면 → 동시 쓰기라 이 관측으로는 판정할 수 없다")
