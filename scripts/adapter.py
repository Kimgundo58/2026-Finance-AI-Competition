# -*- coding: utf-8 -*-
"""LLM 어댑터 — 호출 자리(슬롯)별 제공자 라우팅과 f_axis 차단 게이트.

F축(협약·사업계획서·집행내역·인력·거래처·기업정보·수혜이력) 이 프롬프트에 들어가는 호출은
외부 제공자로 나가지 못한다. 게이트는 두 겹이다: 슬롯 선언(`f_axis=True`)과 프롬프트 내 F축 필드명 탐지.
막히면 `tenant.incidents(종류='ROUTING_BLOCK')` 에 남는다.
...
슬롯표: dict[str, 슬롯] = {
    ...
    # (4)-a 조항별 스크리닝. 채택 미결이나 호출 자리는 잡아둔다
    "스크리닝": 슬롯("스크리닝", "(4)-a 조항별 독립 판정 (채택 미결)", True, "Qwen/Qwen3-8B"),
    # (4)-b 판정 조립. F 프로필이 들어가므로 외부 배치 불가
    "판정조립": 슬롯("판정조립", "(4)-b 최종 판정 + 인용", True, "Qwen/Qwen3-32B-AWQ", 1500),
    # (5) 표현만. 판단불가 경로 전용. F축이 안 들어가므로 외부 라우팅이 가능한 유일한 슬롯이나
    #     프롬프트 검사는 그대로 탄다. 모델은 vLLM 에 한 모델만 띄우므로 32B-AWQ 로 맞춘다
...
# 컬럼명·API 필드명·화면 라벨을 함께 본다. 한 단어짜리 흔한 말(‘금액’ 등)은 오탐 때문에 넣지 않는다.
...
def F축_흔적(텍스트: str) -> list[str]:
    """프롬프트에서 F축 필드의 자취를 찾는다. 매칭 원문이 아니라 필드명만 돌려준다 — incidents 에 실데이터가 실리면 안 된다."""
...
def 사고흔적_걷기(내용: str | None) -> tuple[str, dict]:
    ...
    `normalize_run.py` 도 이 함수를 쓴다 — 복사하지 않는다. 여는 태그 없이 닫는 태그만 오는 경우가 있어
    `</think>` 기준으로 자른다.
    """
...
# vLLM 주소 해석 — `ops.gpu_pod` 우선, env 폴백. 매 호출 DB 왕복을 피하려고 TTL 캐시를 둔다.
...
def vllm_url(*, 강제갱신: bool = False) -> str:
    ...
    try:
        ...
        # DB 나 표가 없어도 판정이 죽으면 안 된다. 조용히 env 로 간다.
        값 = env폴백
...
def _재시도가능(e: Exception) -> bool:
    """요청이 서버에 닿기 전에 끊긴 실패(URLError·ConnectionError)만 True.

    읽기 타임아웃과 HTTPError 는 이미 생성이 시작됐을 수 있어 재시도하지 않는다.
    HTTPError 는 URLError 의 서브클래스라 isinstance 순서를 지킨다.
    """
...
@dataclass
class LocalVLLM(제공자):
    """자체/임대 GPU 의 vLLM (OpenAI 호환). 주소는 `vllm_url()`.

    RunPod 는 우리가 띄운 프로세스라 외부 제공자로 치지 않는다. `SUDDOE_VLLM_SELF_HOSTED=0` 이면 외부로 본다.
    """
    이름: str = "LocalVLLM"
    ...
    def 호출(self, 프롬프트, 스키마, s, *, 온도, 타임아웃, 재시도: int = 1):
        본문 = {
            ...
            "temperature": 온도,          # 기본 0 — 재현성
            "max_tokens": s.max_tokens,
        }
        if 스키마:
            # 최상위 키로 둔다. `extra_body` 로 감싸면 HTTP 직호출에서 조용히 버려진다
            본문["guided_json"] = 스키마
        req = urllib.request.Request(
            f"{self.url}/v1/chat/completions",
            data=json.dumps(본문, ensure_ascii=False).encode(),
            headers={"Content-Type": "application/json",
                     # UA 가 없으면 RunPod 앞단 Cloudflare 가 403 을 돌려준다. 지우지 않는다
                     "User-Agent": "suddoe/1.0"})
        # 서버에 닿기 전에 끊긴 경우만 재시도한다(`_재시도가능`)
        for 회차 in range(재시도 + 1):
            ...
            try:
                ...
                # json.loads 앞에서 사고 흔적을 걷고, 그 사실을 메타에 싣는다
                내용, 사고사실 = 사고흔적_걷기(내용)
...
class AnthropicAPI(제공자):
    """비활성 대체 경로. `SUDDOE_ALLOW_EXTERNAL=1` 로 살려도 f_axis 게이트가 F축 슬롯을 막는다."""
    이름: str = "AnthropicAPI"
    외부: bool = True
...
class LLMAdapter:
    ...
    def __init__(self, 제공자들: list[제공자] | None = None, *, dsn: str | None = None,
        ...
        self.dsn = dsn or DSN
        # `기록=False` 는 자가검사 전용. 운영 경로는 항상 True
        self.기록 = 기록
    ...
    def _고르기(self, s: 슬롯, 강제: str | None) -> 제공자:
        ...
        # 강제 지정이면 활성 여부와 무관하게 그것을 고른다 — 게이트 테스트가 비활성 외부 제공자를 지목한다
        if 강제:
            return 후보[0]
    ...
    # 타임아웃 240 은 프록시를 안 타는 경로용 상한. RunPod 프록시 경유는 그 전에 HTTP 524 로 끊긴다
...
def _selftest() -> int:
    ...
    기록 = "--record" in sys.argv       # 기본은 DB 에 안 남긴다
