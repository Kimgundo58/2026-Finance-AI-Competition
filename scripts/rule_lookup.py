...
def _엄격병합(rows: Sequence[dict], *, 허용층우선: str | None = None) -> dict:
    ...
    def _조건합() -> tuple[str | None, list[str]]:
        """사전승인_조건은 «스칼라» 라 지금까지 `next(...)` 로 **하나만** 살았다.

        🔴 2026-09-06 실측 — 값이 2개 이상인 21개 조합 «전부» L1 이 이기고 L2 가 버려졌다
        (예외 0건). 그래서 예비창업의 "점검항목 1개 이상 부적정이면 집행 불가" 는
        프롬프트에 **한 번도 간 적이 없다**.

        L1 과 L2 는 «충돌» 이 아니라 «다른 사실» 이다 — L1 은 "2천만원 초과 시 소위 심의",
        L2 는 "점검항목 부적정이면 집행 불가". 어느 쪽도 다른 쪽을 대체하지 않는다.
        precedence 로 풀 문제가 아니라 **둘 다 실어야** 하는 문제다. 같은 함수 안에서
        `증빙`은 이미 UNION 이고 `한도`는 min 인데 이 칸에만 규칙이 없었다.

        순서는 **L1 → L2 → L3** (일반 → 특수). rows 가 `ORDER BY layer` 라 그대로 쓴다.
        중복은 **문장 단위**로 뺀다 — 464(창업도약)처럼 L1 의 단서를 이미 제 문장으로
        갖고 있는 행이 있어서, 통째 비교로는 안 걸리고 같은 문장이 두 번 실린다.
        """
        문장들: list[str] = []
        본: set[str] = set()
        층: list[str] = []
        for r in rows:
            원문 = (r.get("사전승인_조건") or "").strip()
            if not 원문:
                continue
            기여 = False
...
def _오버레이(eff: dict, l3: dict) -> dict:
    ...
    _더 = [문.strip() for 문 in re.split(r"(?<=\.)\s+", l3.get("사전승인_조건") or "")
