...
def _조건_리스트화(v) -> list[str]:
    """`사전승인_조건` 을 항상 `list[str]` 로 맞춘다.

    2026-09-06(사전승인_조건 배열화) — DB 컬럼이 TEXT -> TEXT[] 로 바뀐다(마이그레이션은
    중앙이 친다). 여기서 «둘 다» 받는다: 마이그레이션 «전»(psycopg 가 str 을 준다)이면
    한 원소짜리 배열로 감싸고, 마이그레이션 «후»(psycopg 가 list 를 준다)면 그대로 쓴다.
...
def _l3정규화(l3: dict | None) -> dict | None:
    ...
    r = {
        ...
        # 🔴 2026-09-06 — 배열화. `l3_load.l3룰()` 이 이제 리스트를 준다(l3_load.py:264).
        #    문자열이 넘어와도(구버전 호출부 등) `_조건_리스트화()` 가 문자 단위 폭발을 막는다.
...
def _예시_문장(라벨: str, 목록: Sequence[str] | None, 최대: int = _예시_최대개수,
    ...
    🔴 2026-09-06 오너 결정 — `조건부보존=True`(금지예시 전용)면 기준이 «바뀐다».
    「토큰으로 자른다」가 아니라 **「안전망이 있는 것만 자른다」** 다.
      · `무조건` 금지는 게이트 A(`금지적중`)가 DB 배열을 «직접» 훑어 어차피 잡는다.
        프롬프트에서 가려져도 즉답 불가 경로가 살아 있다 -> 잘라도 된다
      · `예외단서` 가 붙은 것(조건부)은 게이트 A 가 «일부러» 안 잡는다. 여기서 가리면
        안전망이 «없다» — LLM 이 「제약 없음」으로 읽는다. 방향이 «틀린 가능» 쪽이다
    실측(2026-09-06): 65조합에서 가려지던 금지예시 90개 = 무조건 79 · 조건부 11 · 애매 0.
    조건부 11 을 전부 실어도 +450자다(캡ON 11,024 -> 11,474자).

    🔴 "외 N종" 은 «무조건분만» 센다 — 조건부를 다 실은 뒤 전체 기준으로 찍으면 숫자가 안 맞는다.
    🔴 허용예시엔 «쓰지 않는다» — 조건부/무조건 판별기가 허용예시엔 아예 없다(`금지적중`의
       대칭이 코드에 없다는 것을 레인 K 가 grep 0건으로 확인). 판별기가 없으면 이 기준을 못 댄다.
    """
    if not 목록:
        return None
    if not 조건부보존:
        ...
        남음 = max(0, len(무조건) - 최대)              # 🔴 잘리는 것은 «무조건분뿐» 이다
        살린무조건 = 무조건[:최대]
...
def _엄격병합(rows: Sequence[dict], *, 허용층우선: str | None = None) -> dict:
    ...
    def _조건합() -> tuple[str | None, list[str]]:
        """사전승인_조건은 «스칼라» 라 예전엔 `next(...)` 로 하나만 살았다. 2026-09-06 에
        문장 단위 UNION 으로 고쳤고(커밋 8ae285f), 같은 날 컬럼 자체가 TEXT -> TEXT[] 로
        바뀐다(마이그레이션은 중앙). 이 함수는 그 «배열» 을 받는 쪽으로 다시 고친다.

        🔴 2026-09-06 실측(배열화 전) — 값이 2개 이상인 21개 조합 «전부» L1 이 이기고
        L2 가 버려졌다(예외 0건). 예비창업의 "점검항목 1개 이상 부적정이면 집행 불가"
        가 프롬프트에 «한 번도 간 적이 없었다» — 그래서 UNION 으로 고쳤다.

        🔴 **원소 단위가 아니라 «문장 단위» 로 중복을 뺀다 — 그대로 유지한다.**
        마이그레이션은 «내용 변환 없음» 이 조건이다 — 값 있는 31행을 «옛 스칼라 문자열
        통째로» 한 원소짜리 배열에 담는다. 그 원소 하나에 문장이 여러 개 있을 수 있다
        (예: "2천만원 초과 시 사전심의. 부적정 1개 이상이면 불가."). 원소 단위로만
        중복을 빼면 이 두 문장이 «하나의 조건» 취급돼 부분 중복을 못 잡는다. 그래서
        배열을 먼저 펼치고, 원소 «안에서도» 문장 단위로 다시 쪼갠다 — 옛 스칼라
        문자열을 직접 쪼개던 것과 «수학적으로 동일한» 결과를 낸다(1원소 배열을 펼쳐
        그 안에서 문장을 쪼개는 것 = 원문 문자열을 바로 쪼개는 것). 그래서 마이그레이션
        «전/후» eff["사전승인_조건"] 이 지금 데이터에서 완전히 같다 — 검산으로 확인했다
        (scratchpad/배열화_설계와SQL.md).
        ...
        층: list[str] = []
        for r in rows:
            원소들 = r.get("사전승인_조건") or []       # 🔴 이제 리스트다 — 문자열이 아니다
            기여 = False
            for 원문 in 원소들:
                원문 = (원문 or "").strip()
                if not 원문:
        ...
        #    다음 층의 문장과 «한 문장으로 붙어» 읽는 쪽이 층 경계를 못 본다. 종결부호를 채운다.
    ...
    # 🔴 2026-09-06 — `조건층` 도 더한다. 사전승인_조건 문장을 «실제로 준» 층이라
    #    위 주석의 같은 이유로 구속에 기여한다 (전엔 그 값이 하나만 살아서 셀 것도 없었다).
...
def _오버레이(eff: dict, l3: dict) -> dict:
    ...
    # 🔴 2026-09-06 — 여기도 first-non-null 이었다(`eff or l3`). L3 조건이 있으면 base 가
...
def _self_test() -> int:
    ...
    # 🔴 2026-09-06(오너 결정) — 캡 «절충». 기준이 「토큰」이 아니라 「안전망이 있는가」다.
    #    무조건 금지는 게이트 A 가 어차피 잡으니 잘라도 되고, 예외단서가 붙은 조건부는
    #    게이트 A 가 «일부러» 안 잡아 안전망이 없으니 «항상 전부» 싣는다.
    #    오너가 지정한 케이스: 조건부 2 + 무조건 12 -> 표시 10개 + "외 4종"
    ...
    # 🔴 "외 N종" 은 «무조건분만» 센다 — 전체(14) 기준이면 "외 6종" 이 돼 숫자가 안 맞는다
    ...
    # 🔴 판별기의 «사정거리» 를 시험으로 박아둔다 — 이 캡은 `금지예시_해부()` 에 «전적으로»
    #    매여 있고, 그 판별기는 «괄호 안» 의 (예외|제외|집행 제한) «만» 본다(:104).
    #    "다만 승인 시 가능"·"원칙적으로 불가" 는 사람 눈엔 조건부인데 «무조건» 으로 떨어진다.
    #    -> 그런 항목은 캡에 잘릴 수 있다. «그래도 안전망은 있다» — 게이트 A 도 «같은»
    #       판별기를 쓰므로 무조건으로 보고 즉답 불가로 잡는다. 판별이 틀려도 «캡과 게이트가
    #       같이 틀려서» 구멍이 안 생긴다. 판별기를 넓히려면 «둘을 같이» 넓혀야 한다.
    ...
    def R(layer, 허용, 한도=None, rid=0, ver=True):
        # 🔴 배열화 — 「사전승인_조건」이 이제 `list[str]` 이다(실 DB 행 모양과 맞춘다).
    ...
    # ══════════════════════════════════════════════════════════════════
    # 🔴 2026-09-06 — 사전승인_조건 배열화 자가검사
    # ══════════════════════════════════════════════════════════════════
    # 1. _조건_리스트화 — 문자열(마이그레이션 전)·배열(후)·None 셋 다 안전하게 받는가
    ...
    # 2. _조건합 — 배열 «입력» 이 예전(문자열 직접 분할)과 «같은 결과» 를 내는가
    #    (마이그레이션은 "값 있는 31행을 옛 문자열 통째로 1원소 배열에 담는다" 이므로
    #    이 등가성이 "배열화 전/후 eff 가 같은 내용" 검산의 핵심이다)
    ...
    # 3. 서로 다른 두 층의 조건이 «둘 다» 실리는가(UNION, 8ae285f 가 고친 것 — 안 깨졌는지)
