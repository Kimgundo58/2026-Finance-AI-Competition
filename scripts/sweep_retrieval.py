...
실행:
    PYTHONIOENCODING=utf-8 python scripts/sweep_retrieval.py            # 전체 그리드
    PYTHONIOENCODING=utf-8 python scripts/sweep_retrieval.py --빠름     # 축소 그리드
    PYTHONIOENCODING=utf-8 python scripts/sweep_retrieval.py --no-log
"""
...
후보K_그리드 = [20, 50, 100]
필터_그리드 = [False, True]
통과조건_그리드 = [0.00, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60]
...
def 통과조건분해(순위별, 통과조건값들: list[float]) -> dict:
    ...
    n = len(순위별) or 1
    for τ in 통과조건_그리드:
        통과 = [(r, a) for (r, a), g in zip(순위별, 통과조건값들) if g >= τ]
        차단 = [(r, a) for (r, a), g in zip(순위별, 통과조건값들) if g < τ]
        out[f"τ={τ:.2f}"] = {
            "통과": len(통과),
...
def main() -> None:
    ...
    with psycopg.connect(DSN) as conn:
        ...
        if not 문항:
            sys.exit("평가 대상 0건. scripts/pin_golden_chunks.py 를 먼저 돌려라.")
        ...
        print(f"그리드 W{len(W들)} × RRF_K{len(RRFK들)} × 후보K{len(후보K들)} × 필터2 "
              ...
              f"(τ {len(통과조건_그리드)}점은 각 조합의 부가 지표)")
    ...
    # 통과 조건 τ 판독은 최고 조합 기준으로 한 번만 낸다 (전 조합은 eval.runs 에 있다)
    print("\n게이트 임계 τ — 최고 조합 기준")
