...
def _강등코드_소스훑기() -> set[str]:
    ...
    """
    여기 = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # scripts/
    본: set[str] = set()
    ...
    def _(*a):
        try:
            ...
            r = subprocess.run(a, capture_output=True, text=True,
                               ...
                               cwd=os.path.dirname(os.path.dirname(os.path.abspath(__file__))), timeout=10)
...
def _토큰합(원출력: dict) -> tuple[int, int, int]:
    """🔴 2026-09-08(ai-33 QA — 토큰·비용) — 이 문항의 (입력토큰, 출력토큰, 추론content길이).

    데이터는 이미 있다(`orchestrate.판정()` 이 `모델.토큰`·`모델.정규화메타.토큰` 에 담아
    왔다) — 여기서 새로 «캡처» 하는 게 아니라 두 LLM 호출(정규화 1회 + 판정 1회, 게이트C
    가 열리면 더 늘 수 있지만 그건 안 센다 — run195 주석 참고)의 토큰을 «합산» 만 한다.
    추론content길이(<think> 로 빠진 안 보이는 사고 글자수)도 두 단계 합으로 같이 뗀다 —
    2026-09-07 실측: thinking 켠 채로 출력 2,466토큰 중 2,300이 안 보이는 사고였다.
    """
    모델 = 원출력.get("모델") or {}
    판정토큰 = 모델.get("토큰") or {}
    정규화토큰 = (모델.get("정규화메타") or {}).get("토큰") or {}
    입력 = (판정토큰.get("prompt_tokens") or 0) + (정규화토큰.get("prompt_tokens") or 0)
    출력 = (판정토큰.get("completion_tokens") or 0) + (정규화토큰.get("completion_tokens") or 0)
    정규화메타 = 모델.get("정규화메타") or {}
    추론길이 = (모델.get("추론content길이") or 0) + (정규화메타.get("추론content길이") or 0)
    return 입력, 출력, 추론길이
...
def _부분집합표() -> tuple[str, dict]:
    경로 = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
...
def 실행(*, dry: bool, limit: int | None, 세트: list[str] | None, 라벨: str | None,
    ...
    # 🔴 2026-09-08(ai-33 QA — 토큰·비용) — 단가는 여기서 «한 번만» 읽어 지표()·설정
    #    둘 다에 «같은 값»을 쓴다. 코드에 숫자를 안 박는다 — 이 세션은 확정된 계약
    ...
    def 지표(부분: list[dict]) -> dict:
        ...
        return {
            ...
            #    🔴 키 이름에 «존재기반» 을 박는다 — 나중에 누가 이걸 "문구 정확도"로
            #    읽으면 또 부풀림이 된다(ai-33 지적).
            "해야할일_채택률_존재기반": (round(
                sum(1 for it in 부분
            ...
            #    안 바뀌게.
            "입력토큰": sum(_토큰합(it["원출력"])[0] for it in 부분),
            "출력토큰": sum(_토큰합(it["원출력"])[1] for it in 부분),
            "추론content길이_합": sum(_토큰합(it["원출력"])[2] for it in 부분),
            "추정비용USD": (round(
                sum(_토큰합(it["원출력"])[0] for it in 부분) / 1_000_000 * _단가.get("in", 0)
                + sum(_토큰합(it["원출력"])[1] for it in 부분) / 1_000_000 * _단가.get("out", 0), 4)
                if _단가 else None),
    ...
    설정 = {"dry": dry, "top_k": top_k, "세트": 세트, "limit": limit,
          ...
          # 🔴 그날 쓴 단가를 이 run 에 못박는다 — 나중에 단가가 바뀌어도 이 run 의
          #    추정비용USD 가 소급으로 안 틀려진다. 미설정이면 빈 dict(추정비용USD=None).
          "단가": {**_단가, "단위": "USD/1M토큰"} if _단가 else {"미설정": True},
          "묶음": _묶음정의,
          "git": _헤드(),
