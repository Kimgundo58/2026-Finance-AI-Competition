...
무너진다. 물리 분리(다른 테이블) + 함수 통과 조건(이 파일)가 짝이다.
...
#
# 실측 (2026-09-01 · 정답셋 77문항 × 적재분 193사례, top-1 코사인):
...
# ── 자기 점검 (F5·F7) ────────────────────────────────────────────────────────
def _selftest() -> int:
    """`판정` 통과 조건과 분리 불변식을 스스로 검사한다. 회귀에서 이걸 돌린다."""
    실패 = 0
    with db.connect() as conn, conn.cursor() as cur:
        # ① 통과 조건 — 판단불가 아닌 판정은 전부 거부
...
def _f7() -> int:
    ...
    for 파일, 금지, 왜, 판정경로만 in [
        ...
        # 판정 중 실제로 실행되는 부분만 본다. 파일 하단의 자기 점검 자동 점검은
    ...
    # ② 통과 조건 — 4-way 중 판단불가 외 전부 거부. ①과 달리 실행으로 본다
    with db.connect() as conn, conn.cursor() as cur:
        for p in ("가능", "조건부", "불가"):
            try:
                유사사례(cur, "테스트", 판정=p)
