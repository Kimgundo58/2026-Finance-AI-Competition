...
골든셋 77문항의 정답근거 82건이 **전부 L1·L2 이고 L3 는 0건**이다. HWPX/HWP 파서를
...
실행:
    PYTHONIOENCODING=utf-8 python scripts/seed_l3_fixture.py            # 적재
    PYTHONIOENCODING=utf-8 python scripts/seed_l3_fixture.py --verify   # 검증만
    PYTHONIOENCODING=utf-8 python scripts/seed_l3_fixture.py --drop     # 픽스처 제거
"""
...
#           폴백 경로(상위 우선 + 동일층 엄격)로 가서 L3 오버레이가 실제로 작동한다
#
    ...
    },
    {
        ...
        "조문": [
            ("제1조", "목적", "제1장 총칙", 1,
             ...
             "사항을 정함을 목적으로 한다."),
            # 🔴 해소되지 않는 참조 — dangling 을 **업로드 시점에** 알리는 경로를 태운다
            ...
            # ── 게이팅 (2) 참조만 · 해소 1건 + dangling 1건이 한 조문에 섞인 경우 ──
...
def 적재(conn) -> None:
    with conn.cursor() as cur:
        for org in 기관들:
            ...
            # 🔴 dangling 은 판정 시점이 아니라 **업로드 시점**에 센다 (CLAUDE.md).
...
def _org입력검증(cur) -> list[str]:
    """🔴 골든셋 77문항에는 `org_id` 가 없다. 게스트 경로가 판정을 죽이면 안 된다.
