...
def 판정(질문: str, *, 사업명: str | None = None, org_id=None, dry: bool = False,
    ...
    `사용자F값` : B(2026-09-06, 레인 Y) — 화면에서 «방금» 받은 심층질문 답. `b5_값()` 과
                 **같은 컬럼명 모양**(`{"정부지원_현금": 1000000, ...}`)이어야 한다 —
                 `f값_경로키()` 로 감싸는 자리가 (5) 전제해소 한 곳뿐이도록 호출부에서
                 미리 dotted-path 로 바꾸지 않는다.
                 🔴 **DB 저장값(`_f사실`)보다 우선한다** — 사람이 이 화면에서 방금 답한
                 값이 DB 에 박힌 값보다 최신이다(DB 값은 지난 판정 시점 스냅샷일 수
                 있다). 병합 순서가 그래서 `_f사실` 을 먼저 깔고 `사용자F값` 으로 덮는다.
                 None 이면 **바이트 단위로 종전과 같다** — 안 넘기는 모든 호출부(평가
                 하네스 포함)는 이 변경에 안 흔들린다.
    """
    t0 = time.time()
    지연: dict[str, int] = {}
    경로: list[str] = []
    ...
    try:
        ...
        # 🔴 B(2026-09-06, 레인 Y) — 사용자가 방금 답한 값이 DB 스냅샷보다 최신이라
        #    `_f사실`(DB) 을 먼저 깔고 `사용자F값`(화면) 으로 덮는다. `사용자F값=None`
        #    이면(호출부가 안 넘기면) 이 줄은 `f값_경로키(_f사실)` 과 바이트 단위로 같다.
        f값 = f값_경로키(_f사실)
        if 사용자F값:
