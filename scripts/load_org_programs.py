...
def 구분정리(g: str) -> str:
    """🔴 낱글자 사이에 낀 자간 공백을 없앤다 — 「경 남」·「대 전」·「충 북」.

    모두의창업 통합공고 원문 헤더가 `□ 경 남 : 6개` 라 자간이 그대로 딸려온다.
    **추출은 정확하고 정규화가 빠진 것**이라 여기서 편다. 지역으로 묶는 순간 어긋난다.
    `구분` 은 PK 의 일부라 이걸 적재 «후» 에 고치면 PK 가 바뀐다 — 들어올 때 편다.
    """
    if not g:
        return g
    g = re.sub(r"(?<=[가-힣]) (?=[가-힣](?![가-힣]))", "", g)
...
def 다듬기(레코드: list[dict]) -> tuple[list[dict], Counter, list[dict]]:
    ...
    for r in 레코드:
        ...
        if 고침:
            버림[f"정정:{이름}→{고침}"] += 1
            이름 = 고침
        # 🔴 `구분` 은 «섹션» 축이다 — 일반형·특화분야·기술분야·차수. PK 의 일부라
        #    여기에 다른 축의 값이 들어오면 **같은 기관이 두 행이 된다.**
        #    공공데이터포털 CSV 는 `기관구분`(전부 '대학')을 준다. 그건 기관의 속성이지
        #    섹션이 아니다. 실제로 그 CSV 9곳은 2023년 hwpx 9곳과 **완전히 같은 기관**이고,
        #    CSV 가 더 주는 것은 «주소» 뿐이다. 섹션으로 넣으면 9행이 중복된다.
        구분 = 구분정리((r.get("구분") or "")[:200])
        if str(r.get("출처") or "").lower().endswith(".csv"):
            구분 = ""
        out.append({
            "org_id": org_id(이름),
            "사업명": 사업정본,
...
def main() -> None:
    ...
    # ── 같은 행을 여러 출처가 나눠 준 경우를 **입력 단계에서** 합친다 ──
    #    🔴 같은 (기관·사업·연도·역할·구분) 인데 파일이 다르면 «중복» 이 아니라
    #       «같은 사실을 나눠 적은 것» 이다. 실물 사례: 창업중심대학 2023 을
    #       hwpx 명부는 연락처로, 공공데이터포털 CSV 는 주소로 준다.
    #       비어 있는 칸만 채운다 — 이미 있는 값은 안 덮는다(먼저 온 것이 이긴다).
    #       DB 의 ON CONFLICT DO UPDATE 와 같은 규칙이라 결과가 어긋나지 않는다.
    합침: dict[tuple, dict] = {}
    합친수 = 0
    for r in 행:
        k = (r["org_id"], r["사업명"], r["기준연도"], r["역할"], r["구분"])
        기존 = 합침.get(k)
        if 기존 is None:
            합침[k] = r
            continue
        합친수 += 1
        for 칸 in ("연락처", "주소"):
            if not 기존.get(칸) and r.get(칸):
                기존[칸] = r[칸]
        # 어느 파일들이 합쳐졌는지 남긴다 — 되짚기용
        if r["출처파일"] not in 기존["출처파일"]:
            기존["출처파일"] = f"{기존['출처파일']} + {r['출처파일']}"
    if 합친수:
        print(f"■ 같은 행을 여러 출처가 준 것 {합친수}건 합침 (빈 칸만 채움)")
    행 = list(합침.values())
    ...
    with psycopg.connect(DSN) as conn, conn.cursor() as cur:
        ...
        cur.executemany(
            ...
            # 🔴 DO NOTHING 이 아니다. 같은 행을 여러 출처가 조금씩 다르게 채운다 —
            #    hwpx 명부는 연락처를 주고 공공데이터포털 CSV 는 주소를 준다.
            #    **비어 있는 칸만 채운다.** 이미 있는 값은 덮지 않는다(먼저 온 것이 이긴다).
            "ON CONFLICT (org_id,\"사업명\",\"기준연도\",\"역할\",\"구분\") DO UPDATE SET "
            '  "주소"   = coalesce(tenant.org_programs."주소",   EXCLUDED."주소"), '
            '  "연락처" = coalesce(tenant.org_programs."연락처", EXCLUDED."연락처")',
            행)
        conn.commit()
