# -*- coding: utf-8 -*-
"""D3 — 골든셋 정답근거 ↔ chunk_id 고정 (`eval.golden_chunks`).
...
`chunks.text` 에 부분일치로 되짚는다. 그래서
  ...
  · 실패가 **매칭 버그**인지 **코퍼스 결손**인지 갈리지 않는다
...
실행:
    PYTHONIOENCODING=utf-8 python scripts/pin_golden_chunks.py          # 미고정만
    PYTHONIOENCODING=utf-8 python scripts/pin_golden_chunks.py --재고정  # 전건 다시
    PYTHONIOENCODING=utf-8 python scripts/pin_golden_chunks.py --dry     # 쓰지 않고 보기
"""
...
# 🔴 골든셋의 정답근거 좌표는 조문만이 아니다. 실측(2026-08-31) 결과 역추적 실패 12건이
...
def 매칭(cur, doc: str | None, 조번호: str | None, 원문: str | None):
    ...
    if 조:
        ...
        # 부속물 좌표인데 그 doc 에 부속물 청크가 아예 없으면 코퍼스 결손이다.
        cur.execute(
            "SELECT count(*) FROM corpus.chunks WHERE doc_id=%s AND 조번호 !~ '^제[0-9]'", (doc,)
        )
        n = cur.fetchone()[0]
        사유 += (
            f" (부속물 '{부속}' 이 chunks 에 없다 — 이 doc 의 부속물 청크 {n}건. "
            + ("좌표 표기 불일치" if n else "🔴 부속물이 통째로 미적재 — 코퍼스 결손") + ")"
        )
    else:
