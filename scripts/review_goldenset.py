# -*- coding: utf-8 -*-
"""골든셋 기계 검수 — `_골든셋_검수.json`.

`LLM.md` §5 의 골든셋은 전량 `verified=false` 다. 사람 검수 전에 **기계가 확정할 수 있는
것부터 걷어낸다.** 사람이 봐야 하는 것과 기계가 이미 아는 것을 섞어두면 검수가 안 끝난다.

기계가 확정하는 것 (사람 판단 불필요):
    C1  `정답_근거[].doc` 이 코퍼스에 실재하는가
    C2  조번호가 그 문서에 실재하는가
    C3  🔴 `근거_원문` 이 코퍼스 원문에 실재하는가  ← 정답지 오염의 유일한 기계 검출 지점
...
골든셋이 적어둔 조번호와 대조한다 — 조번호 오기와 원문 날조가 한 번에 잡힌다.

실행:
    python scripts/review_goldenset.py
    python scripts/review_goldenset.py --brief    콘솔 요약만 (파일 안 씀)
"""
...
# 골든셋 `doc` 표기 -> 코퍼스 doc_id. 표기 흔들림은 데이터가 아니라 오기다.
DOC_ALIAS = {
...
def 단편들(원문: str) -> list[str]:
    """골든셋은 `…` 로 중간을 생략하고 `/` 로 두 조항을 잇는다. 각각을 따로 찾는다."""
...
def 원문검증(doc: dict, 표: list, 원문: str) -> dict:
    ...
    `근거_원문` 이 붙임·별표 표에만 있는 경우가 있다 — 그건 골든셋 결함이 아니라
