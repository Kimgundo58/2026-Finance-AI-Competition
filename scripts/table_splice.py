...
🔴 **2026-09-05 정정(레인 C, ai-35 배정 사고 — 실측) — 갈아끼우기 전 내용 손실을
   대조한다.** PR #29(2026-09-04 커밋)가 창업중심대학 참고3 「기계장치」 행의
   구입기준 문단(「PC, 사무용 복합기…」부터 「단순저장장치(USB, 외장하드 등)…
   집행 불가」까지)을 통째로 날렸다 — `_tables.json` 자체에 그 문단이 없었다
   (원인은 `extract_tables.py` 의 pdfplumber 셀 인식 실패, 이 파일 소관이 아니다).
   `붙임_교체()` 는 `_tables.json` 의 내용을 무조건 신뢰했는데, 그 신뢰가 깨지면
   («더 나은 걸 아는 경우에만 갈아끼운다»는 원 철학과 반대로) 원문보다 못한
   걸로 갈아끼우는 사고가 난다. 그래서 표본문을 쓰기 **직전** 원문과 대조해
   ([[text_coverage]].`누락_구간`), 표가 통째로 놓친 연속 구간이 있으면 표본문
   뒤에 **원문 슬라이스 그대로**(축자) 이어 붙인다 — 열 구조는 잃지만 인용
   가능한 형태로는 남는다. 1차 시도(어절 다중집합 재조립)는 버렸다 — 이미
   다른 자리에서 소비된 흔한 어절(「비품」·「등」)이 꼭 필요한 자리에서 빠져
   정답셋의 축자 인용과 문자 단위로 어긋났다(gold_id 561 실측). 문자 단위
   `difflib` 매칭 블록으로 원문을 덮어, 안 덮인 구간만 원문 그대로 뽑으면
   구두점·공백까지 원문과 한 글자도 다르지 않다.
   표 자체 대조는 다중집합(순서 무시)을 쓴다 — 표는 원래 프로즈 순서를
   컬럼으로 재배치하므로 순서 대조는 항상 오탐이다(중앙 ai-35 실측 — 순진한
   줄 단위 대조는 259줄 중 59줄이 헛경보였다).
"""
...
def _손실_보강(원문: str, 표본문: str) -> str:
    ...
    🔴 어절 다중집합 재조립(`부족_어절`)이 아니라 연속 구간 축자 복구
    (`누락_구간`)를 쓴다 — 처음 버전(어절 재조립)은 이미 다른 자리에서
    「소비된」 흔한 어절이 필요한 자리에서 빠져 정답셋 축자 인용과 문자
    단위로 안 맞았다(gold_id 561 실측, 레인 C 2차 정정).
    """
