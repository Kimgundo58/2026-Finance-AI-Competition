...
# 🔴 2026-09-07 V8 — 오답 102건 중 73건(72%)이 «조건부로 빨려든다»(가능→조건부 34 ·
#    불가→조건부 25 · 판단불가→조건부 14). 원인을 오답 요약 34건에서 읽었다:
#      gold415 "…여비로 지출할 수 있습니다. 단, 출장 결과보고서와 증빙서류를 제출해야"
#      gold418 "…한도 이내라 가능하나, 4대보험 가입자여야 하고 증빙과 결과보고서를"
#      gold409 "…지급할 수 있으나, 4대보험 가입과 증빙 서류 제출이 «필수»이며"
#    34건 중 24건(71%)의 요약에 공통 절차 단어(증빙·협약기간·비교견적·세금계산서)가 있다.
#    ⇒ 모델은 「집행할 수 있다」를 «이미 알고» 있는데, 뒤에 챙길 서류를 적으면서 판정을
#      조건부로 내린다.
#    설계상 그것들은 「해야할일」 필드가 따로 나른다(check_items 70개). 그런데 B0 의
#    조건부 정의에 «증빙» 이라는 단어가 들어 있어 경계가 무너졌다 — 증빙은 거의 모든
#    지출에 붙으므로 조건부가 «항상 성립하는 답» 이 된다.
...
def 조립(cur, 질문: str, 정규화: dict, *,
    ...
    def 원문블록(제목: str, 항목들: list[tuple[str, int, str, str, str]]) -> str:
        ...
        for 종류, _id, 머리, 본문, 항호 in 항목들:
            # 🔴 2026-09-07 — 한 조 «안에서» 같은 항 기호가 되풀이되는 문서가 있다.
            #    실측: 경상국립대 안내문 article 428 본문이 「① 급여대장 / ② 입금확인증 /
            #    < 협약 종료 시 > / ① 원천징수이행상황신고서 / ② 4대사회보험료 월별납부확인서」
            #    라 ①② 가 «두 벌» 이고 내용이 서로 다르다. 프롬프트에는 `분해()` 가 네 조각을
            #    정확히 싣는데, 검증 단계(`llm_validate._항_추출`)가 `본문.find(항호)` 로
            #    «첫 번째만» 잡아 두 번째 ①의 «인용 원문» 이 급여대장으로 잘못 표시됐다.
            #    판정은 멀쩡했고 «표시» 만 틀렸다. 그래서 몇 번째인지를 같이 싣는다 —
            #    두 번째부터만 `#N` 을 붙이므로 첫 번째는 예전 값과 바이트 단위로 같다.
            _센다: dict[str, int] = {}
            for 항, 조각 in 분해(본문):
                n += 1
                _키 = 항 or 항호
