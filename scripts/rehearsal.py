...
def 한건(질문: str, 사업명: str | None, org_id: str | None, live: bool,
    ...
    try:
        ...
        return {"판정": "판단불가", "요약": f"판정 실패 ({type(e).__name__}: {str(e)[:120]})",
                "해야할일": [], "인용": [], "신뢰등급": None, "경로": None,
                "강등코드": ["REHEARSAL_ERROR"], "지연ms": int((time.time() - t) * 1000)}
    지연 = r.get("지연ms")
...
def main() -> None:
    ...
    with psycopg.connect(DSN) as conn:
        ...
        for i, 행 in enumerate(행들, 1):
            ...
            등급 = 등급표.get(r["판정"], "확인")
            집계[등급] = 집계.get(등급, 0) + 1
            위험금액[등급] = 위험금액.get(등급, 0) + float(행["금액"])
            결과.append({**행, "질문": q, **r, "리스크": 등급})
            print(f"  [{등급:2}] {i:>3}/{len(행들)} {행['비목']:<10} "
        ...
        for 등급 in ("높음", "보통", "확인", "낮음"):
            if 등급 in 집계:
                print(f"  {등급:<3} {집계[등급]:>3}건  {int(위험금액[등급]):>12,}원  "
                      f"({위험금액[등급]/총액*100:4.1f}%)")
        재원 = {}
