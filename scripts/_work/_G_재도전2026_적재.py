...
───────────────────────────────────────────────────────────────────
  ...
  · 별표 부재 : 제17조②가 「공무원 여비 규정」[별표 1] 을 참조하는데 이 PDF 9쪽 안에
    별표가 없다 → dangling 참조. 여비 한도를 NULL 로 두고 조건 문장에만 적었다(G1 룰).

알려진 전사 결손 1건 — 판정 룰과 무관
  · 제29조(창업기업등 최종평가) ③ 말미가 잘려 있다("…①과 ②번 항목 중 하나"). 최종평가
    조문이라 사업비 비목 룰이 인용하지 않는다. 룰 근거로 쓰지 말 것.

실행:  PYTHONIOENCODING=utf-8 python scripts/_work/_G_재도전2026_적재.py          # 미리보기
       PYTHONIOENCODING=utf-8 python scripts/_work/_G_재도전2026_적재.py --commit  # 실적재
"""
