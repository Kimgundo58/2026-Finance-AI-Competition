...
`review_rules.py` 는 사람이 눈으로 보는 도구고, 이 파일은 그 앞단의 기계 게이트다.
...
실행:
    PYTHONIOENCODING=utf-8 python scripts/verify_rules.py            # 요약
    PYTHONIOENCODING=utf-8 python scripts/verify_rules.py --show 0.5 # 임계 미만 전부 출력
    PYTHONIOENCODING=utf-8 python scripts/verify_rules.py --json scripts/_work/_rules_검증.json
"""
