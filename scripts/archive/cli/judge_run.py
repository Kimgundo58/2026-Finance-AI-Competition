...
세 조각을 잇는다. 각 조각은 다른 파일이 소유한다:
    ...
    scripts/archive/eval/score_judgment.py     채점                           (중앙)
...
실행:
    PYTHONIOENCODING=utf-8 python scripts/archive/cli/judge_run.py --isolated --limit 5
    PYTHONIOENCODING=utf-8 python scripts/archive/cli/judge_run.py --isolated --out 결과.jsonl
"""
...
def main() -> None:
    ...
    print(f"채점:  PYTHONIOENCODING=utf-8 python scripts/archive/eval/score_judgment.py --in {a.out}")
