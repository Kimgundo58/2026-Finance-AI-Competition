...
사용:
    python scripts/archive/cli/runpod_pod.py ls
    python scripts/archive/cli/runpod_pod.py open --gpu "RTX 2000 Ada" --hours 1 --template-id <id>
    python scripts/archive/cli/runpod_pod.py close            # 목록의 팟 전부
    python scripts/archive/cli/runpod_pod.py close <pod-id>   # 하나만
"""
...
def resolve_gpu(name: str) -> str:
    ...
    sys.exit(f"GPU '{name}' 를 못 찾았다." + (f" 비슷한 것: {near}" if near else
             " `python scripts/archive/cli/pick_gpu.py --task <t>` 로 이름을 확인할 것."))
...
def cmd_open(args) -> None:
    ...
    if not args.gpu:
        sys.exit("--gpu 가 필요하다 (CPU 팟이면 --cpu 를 줄 것). "
                 "이름은 `python scripts/archive/cli/pick_gpu.py --task <t>` 로 확인한다.")
    ...
    print(f"  닫기:  python scripts/archive/cli/runpod_pod.py close {pid}")
...
def cmd_close(args) -> None:
    ...
    if still:
        print(f"\n🔴 아직 남아 있다: {still}")
        print("   몇 초 뒤 `python scripts/archive/cli/runpod_pod.py ls` 로 다시 볼 것. "
