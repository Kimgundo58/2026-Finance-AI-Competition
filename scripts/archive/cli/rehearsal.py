...
🔴 **`확인` 과 `점검` 을 섞으면 리허설이 거짓말을 한다.** 2026-09-01 실전 E2E 에서
   ...
   두 곳이 다른 기준을 쓰면 회귀 통과 조건과 리허설이 서로 다른 말을 한다.

실행:
    PYTHONIOENCODING=utf-8 python scripts/archive/cli/rehearsal.py --profile <uuid>
    PYTHONIOENCODING=utf-8 python scripts/archive/cli/rehearsal.py --fixture     # f_exec 0행일 때
    PYTHONIOENCODING=utf-8 python scripts/archive/cli/rehearsal.py --profile <uuid> --live   # A9 이후
"""
