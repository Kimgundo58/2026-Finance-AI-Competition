# -*- coding: utf-8 -*-
"""화면 9 문의 초안 — LLM 호출 자리 ⑤ (`LLM.md` §1 · `Agent.md` §7).

판정이 `판단불가` 로 끝났을 때, 사용자가 **주관기관 담당자에게 보낼 문의 메일 본문**을
만든다. 호출 자리 ⑤ 는 "표현만" 하는 호출 자리이라 **"판정 1건 = LLM 2회" 셈 밖**이다 —
판단불가 건에서만 한 번 더 부른다.

🔴 이 호출 자리는 판정하지 않는다. 결론을 새로 만들면 그 순간 "판정 1건 = LLM 2회" 가
   깨지고, 판단불가로 떨어진 건에 근거 없는 답이 붙는다. 호출 자리 ⑤ 가 하는 일은
...
`생성()` 은 A 가 호출 자리 클라이언트를 넘겨주면 그대로 돌아간다.

실행
────
    PYTHONIOENCODING=utf-8 python scripts/archive/cli/draft_inquiry.py --dump
    PYTHONIOENCODING=utf-8 python scripts/archive/cli/draft_inquiry.py --dump --from-db <decision_id>
"""
...
# ── 호출 자리 ⑤ 시스템 지시 — 🔴 고정 문구. 가변 값 금지 (prefix 캐시 안정 구간) ──
...
# 호출 자리 ⑤ 출력 스키마. guided_json 은 **최상위** 인자로 넘긴다 (`extra_body` 는
...
def 프롬프트(판정결과: dict) -> list[dict]:
    """호출 자리 ⑤ messages. B0 고정 → <자료> 래퍼 → 지시 순서."""
    판정 = 판정결과.get("판정")
    if 판정 != "판단불가":
        raise RuntimeError(
...
def 검증(출력: dict, 판정결과: dict) -> tuple[bool, list[str]]:
    """호출 자리 ⑤ 출력 게이트. 실패하면 초안을 버리고 정적 템플릿으로 떨어진다.
...
# ── 대체 경로 — LLM 없이도 화면 9 가 빈칸이 되지 않는다 ─────────────────────────
...
def 생성(판정결과: dict, 호출=None) -> dict:
    """문의 초안. `호출` 은 A 의 호출 자리 클라이언트 — `(messages, guided_json) -> dict`.
