...
미결 목록 #10 "1만 청크 초과 시 ANN 재검토 — 실측" 의 실행부.
...
🔴 **질의 벡터는 반드시 실제 질문을 임베딩해서 쓴다** (`eval.golden_set.질문`).
   초판은 규정 모음 청크의 임베딩을 질의로 재사용했는데 **결과가 완전히 뒤집혔다** (2026-08-31):

     청크를 질의로 (틀린 방법)   ef=100  recall 99.5%
     실제 질문을 질의로 (맞음)   ef=100  recall 90.9%, 최악 64.0%

   질의가 규정 모음 안의 벡터와 동일하면 HNSW 그래프가 곧장 수렴해서 재현율이 부풀려진다.
...
**규정 모음이 크게 늘면 이 스크립트로 다시 잰다.**

실행:
    PYTHONIOENCODING=utf-8 python scripts/archive/eval/bench_ann.py
    PYTHONIOENCODING=utf-8 python scripts/archive/eval/bench_ann.py --build      # HNSW 생성까지
    PYTHONIOENCODING=utf-8 python scripts/archive/eval/bench_ann.py --drop       # 인덱스 제거
    PYTHONIOENCODING=utf-8 python scripts/archive/eval/bench_ann.py --chunk-queries  # 옛 방법(비권장)
"""
