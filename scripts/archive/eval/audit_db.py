...
실행:
    PYTHONIOENCODING=utf-8 python scripts/archive/eval/audit_db.py
    PYTHONIOENCODING=utf-8 python scripts/archive/eval/audit_db.py --verbose   # 정상 항목도 출력
"""
...
NULL_정상 = {
    ...
    #    고치려면 Stage 0 재파싱이 필요하다 — 후순위. 미결 목록에 올려뒀다.
    ...
    # 🔴 97% NULL 이 결손이 아니라 스키마 모양이다. 사업명은 L2 에만 붙는다 —
    #    L1(공통 규범) 20,030 / L2 495 니까 495/20,525 = 2.4% 만 값이 있는 게 맞다.
    #    판정 검색 필터도 사업명이 아니라 `적용대상` 을 쓴다 (2026-09-02 확인).
    ("corpus.chunks", "사업명"): "L2 에만 붙는다. L1 공통 규범이 20,030/20,525 라 97% NULL 이 정상",
    ...
    #    규정 규정 모음(세부관리기준·통합관리지침)에서 확정 근거를 찾지 못했다.
    ...
    # 🔴 "의도된 NULL" 이 아니라 **정답셋 작성 시 안 채운 필드**다. 오늘 채우지 않는다 —
    #    계약 §10 이 정답셋 문항 수정을 금지한다(정답지를 건드리면 어제와 비교 불가).
    ...
    # 판정 질문 다수가 「살 수 있나요」라 금액이 없다. 한도 비교가 필요한 경우에만 채워진다.
    ("tenant.decisions", "금액"): "금액 없는 질문이 다수다. 한도 비교가 필요한 판정에만 값이 붙는다",
    ...
    #    아니라 **격리에 쓰이는 값**이다 (ai-14 · 작업 갈래 A, 2026-09-01).
