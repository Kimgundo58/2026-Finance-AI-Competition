...
with psycopg.connect(DSN) as conn:
    ...
    if 기존:
        ...
        # 🔴 협약기간은 «미래» 여야 한다. 2024-03-01~2025-02-28 이 박혀 있었는데
        #    그러면 화면의 D-day 자리에 「종료」가 찍혀서 이미 끝난 사업으로 보인다.
        #    이 값을 계산하거나 문서에서 뽑는 경로는 아직 없다 — 시연용 상수다.
        #    ⚠️ 위의 `if 기존:` 이 «건너뜀» 이라, 이 파일만 고쳐서는 **이미 행이 있는
        #       DB 는 안 바뀐다.** 살아있는 DB 는 UPDATE 를 따로 쳐야 한다 (PR 본문 참조).
        row = conn.execute(
            """INSERT INTO tenant.f_profile
                   (org_id, "사업명", "협약시작일", "협약종료일",
                    "정부지원_현금", "자기부담_현금")
               VALUES (%s,%s,'2026-03-01','2026-12-31', 60000000, 15000000)
