...
"""(b) 적재 — Stage 0 산출물을 corpus/tenant/eval 스키마에 넣는다.
  ...
  corpus.documents        문서 목록 (layer · status · index_target · retrieval_scope)
...
실행:  python scripts/archive/seed/load_db.py
"""
...
def main() -> None:
    ...
    for doc_id, d in s0.items():
        layer = d.get("layer")
        path = d.get("path", "")
        # 🔴 인덱싱 경계는 코드가 강제한다 (RAG.md §1). 새 경로는 반드시 이 통과 조건을 탄다.
        ...
        if doc_id in 현행:
            ...
            status = "reference"      # 타부처 보조금 규정 등 — 규정 모음에는 두되 판정 밖
        else:
        ...
        docs.append((doc_id, layer, "창업지원사업", None, None, None, None, status,
                     ...
                     "native", path,
                     index_target,
                     # 좁히기는 정답셋 Recall@5 A/B 후에 한다. 지금은 전부 진입점.
                     "진입점"))
    ...
    with db.connect() as conn, conn.cursor() as cur:
        ...
        #    정답셋만 갱신하려는 것이면 `_work/_골든셋_재적재.py` 를 쓴다.
        ...
        if n청크 and "--청크삭제승인" not in sys.argv:
            sys.exit(
                ...
                "   · 골든셋만 갱신하려면  : python scripts/archive/work/_골든셋_재적재.py --commit\n"
    ...
    #     refs.dst_doc_id   파일경로/약칭 -> refs 참조 확장이 폐포전용 문서에 도달 못 한다
    ...
    할일 = []
    if native == 전체:
        할일.append(("extraction 이 전부 native", "python scripts/archive/indexing/retag_extraction.py --apply"))
    if 진입점 == 전체:
        할일.append(("retrieval_scope 가 전부 진입점", "python scripts/archive/indexing/retag_scope.py"))
    if 경로형:
        할일.append((f"refs.dst_doc_id 미해소 {경로형:,}건",
                     "python scripts/archive/seed/normalize_refs.py --apply"))
    if 버전없음 == 전체:
        할일.append(("documents.version 이 전부 NULL (판정 응답의 버전스탬프를 못 만든다)",
                     "python scripts/archive/indexing/backfill_doc_meta.py --apply"))
    if 할일:
        print("\n" + "=" * 68)
