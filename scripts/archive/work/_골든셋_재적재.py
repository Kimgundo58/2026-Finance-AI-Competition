...
🔴 **`load_db.py` 를 쓰면 안 되는 자리다.** 그쪽은 `TRUNCATE corpus.documents CASCADE`
   ...
   **임베딩 2만여 건이 같이 날아간다.** 정답셋 한 줄 고치자고 GPU 팟을 다시 여는 셈이다.
...
실행:
    PYTHONIOENCODING=utf-8 python scripts/archive/work/_골든셋_재적재.py
    PYTHONIOENCODING=utf-8 python scripts/archive/work/_골든셋_재적재.py --commit
"""
...
def main() -> int:
    ...
    print("\n🔴 다음: PYTHONIOENCODING=utf-8 python scripts/archive/eval/pin_golden_chunks.py --재고정")
    return 0
