...
왜 임베딩을 안 쓰는가
  `scripts/archive/eval/version_diff.py` 는 벡터로 조를 이었다. 그건 **탐색**용이고 이건 **배치**다.
...
실행:
    PYTHONIOENCODING=utf-8 python scripts/archive/agents/agent_a4.py --dry            # 적재 없이 보고만
    PYTHONIOENCODING=utf-8 python scripts/archive/agents/agent_a4.py                  # DB 계보 → 큐
    PYTHONIOENCODING=utf-8 python scripts/archive/agents/agent_a4.py --source xml     # 법령 연혁 → 큐
    PYTHONIOENCODING=utf-8 python scripts/archive/agents/agent_a4.py --source both --heal

## 주 1회 배치 런북 — 이 순서를 지킨다

    python scripts/archive/agents/agent_a4.py --source both --heal    # 개정 감지 → 적재 → 치유 → 조정
    python scripts/archive/agents/agent_a2.py                         # L3 엄격조항 (L3 가 있을 때만)
    python scripts/archive/agents/agent_a5.py --in <기관답변.json>     # 답변이 왔을 때만
...
def main() -> None:
    ...
    Path(a.report).write_text(
        "# A4 개정 대응 실행 보고\n\n> `scripts/archive/agents/agent_a4.py` 산출. 재현성을 위해 LLM 을 쓰지 않는다.\n\n"
        "```\n" + "\n".join(줄들) + "\n```\n", encoding="utf-8")
    print(f"\n보고서 → {a.report}")
