# -*- coding: utf-8 -*-
"""HWP/HWPX → PDF 일괄 변환 (한컴 Office COM). 원본 트리 구조를 `_hwp변환/` 아래에 미러링한다.
...
def sniff(path: Path) -> str | None:
    """매직바이트로 실제 포맷을 판별한다 — OLE → HWP, ZIP → HWPX, 그 외 None."""
...
def main() -> None:
    ...
    for i, src in enumerate(files, 1):
        ...
        t0 = time.time()
        try:
            # 확장자 .hwp 가 실제론 HWPX 인 파일이 많다 — 포맷을 틀리게 주면 Open 이 조용히 False 를 돌려준다.
            fmt = sniff(src)
            if fmt is None:
            ...
            if not saved or not dst.exists() or dst.stat().st_size < 1024:
                # 배포용 문서는 한컴이 저장을 거부한다 — hwp5txt 로 텍스트만 뽑는다 (표는 손실).
