...
실행: python scripts/archive/indexing/build_mss_doc.py
"""
...
def main() -> None:
    ...
    L.append("- 이 문서는 `scripts/archive/indexing/build_mss_doc.py` 가 `_mss_master.json` + `_mss_report.json` 에서 생성한다. **직접 수정하지 말 것.**")
    L.append("")
    L.append("## 1. 수집 현황")
    L.append("")
    L.append("| 항목 | 건수 |")
