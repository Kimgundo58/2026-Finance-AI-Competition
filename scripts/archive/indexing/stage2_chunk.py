...
`build_index.py`(구 규정 모음 기준, 파이프라인 문서 §7-1 "재작성 대상")를 대체한다.
바뀐 것:
...
실행:
    PYTHONIOENCODING=utf-8 python scripts/archive/indexing/stage2_chunk.py --dry-run   # 세보기만
    PYTHONIOENCODING=utf-8 python scripts/archive/indexing/stage2_chunk.py             # 적재
"""
...
#
#    실제 피해: 정답셋 적대적 세트 9문항의 정답 근거가 통째로 인덱스 밖이었다
    ...
    with db.connect() as conn:
        ...
        for doc_id, layer, 기관, pq, ver, status, scope, src in docs:
            ...
            for art in arts:
                ...
                #    · hit@50 70.0->71.4. 근거가 필요하면 refs 참조 확장으로 도달한다 (RAG.md §4-3).
