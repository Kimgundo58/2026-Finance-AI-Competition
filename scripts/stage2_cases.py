...
절차
────
    check    F1 오염검사. **적재보다 먼저 돌린다.** 골든셋 ↔ 사례집 대조
...
왜 `check` 가 먼저인가 (F1)
──────────────────────────
골든셋 적대적 27문항은 **「창업사업화 지원사업 부정행위 방지 사례집」(중기부)의 실제
...
# ════════════════════════════════════════════════════════════════════════════
# 후보 대장 — 문서마다 "어떤 파서로 읽는가"와 "왜 넣는가/빼는가"를 한곳에 적는다.
#
...
def _골든문항() -> list[dict]:
    """골든셋 77문항 + 적대적 초안(사례출처 필드가 여기에만 있다)."""
...
def cmd_load(a) -> None:
    ...
    with psycopg.connect(DSN) as conn:
        with conn.cursor() as cur:
            # 게이트를 적재 직전에 한 번 더. extract 와 load 가 따로 돌 수 있다
