...
def 실행(*, dry: bool, limit: int | None, 세트: str | None, 라벨: str | None,
        ...
        for i, m in enumerate(문항, 1):
            ...
            items.append({
                ...
                "원출력": {
                    ...
                    "실패단계": r.get("실패단계"),
                    "게이트값": 검색.get("게이트값"),
                    # 🔴 dry 에서만 채워진다. `--max-model-len` 이 문항 집합에
                    #    맞는지 재는 유일한 실측치다 (프롬프트가 넘치면 맨 뒤 B6=질문이 잘린다).
                    "프롬프트길이": r.get("프롬프트길이"),
                    "지연ms": r.get("지연ms") or {},
                    "s맵": r.get("s맵") or {},
    ...
    if 길이:
        MML, 자당토큰 = 24576, 0.7          # 한국어 법령문 0.6~0.7 토큰/자 (보수적으로 상단)
        여유 = int((MML - 3000) / 자당토큰)  # max_tokens=3000 을 뺀 입력 예산(자)
        넘 = [x for x in 길이 if x > 여유]
        print(f"프롬프트 자수  중앙 {길이[len(길이)//2]:,} · 최장 {길이[-1]:,} · n={len(길이)}")
...
def main() -> None:
    ...
    ap.add_argument("--세트", choices=["본세트", "적대적", "공식", "보강"])
