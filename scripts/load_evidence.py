...
판정에 쓰이지 않고 화면 안내(발급처 링크·툴팁)에만 쓰이므로 검수 게이트도 없다.
...
🔴 **비목 문자열을 여기서 정규화하지 않는다.**
   ...
   `rules.비목`(어휘집 enum 10종)과의 대조는 별칭 매핑 작업에서 따로 한다.
...
# ════════════════════════════════════════════════════════════════════════════
# 해당비목 → 정본 비목 매핑 (2026-08-31 3차)
# ════════════════════════════════════════════════════════════════════════════
# 정본은 `_비목_어휘집.json` 의 **guided_json_enum 10종**이다 (`비목` 필드가 아니다 —
# 그쪽은 원문 표기라 "기계장치, 공구·기구" 처럼 조인 키로 못 쓴다).
#
# CSV 의 `해당 비목` 은 세 계통이 섞여 있다. 억지로 한 축에 밀어 넣지 않는다.
#   (1) 창업패키지 비목        → 정본 10종으로 매핑
#   (2) 지급수수료 **세목**    → 정본 `지급수수료` 로 접는다 (세목은 별도 축이다. §세목 참조)
...
# (1) 표기 차이 — 원문 표기를 enum 으로. 새로 만들지 않고 어휘집 `비목` 필드와 대조해 얻었다.
_비목_표기별칭 = {
...
#       어휘집에서 적용대상=주관기관이거나, 애초에 비목 축이 아닌 메타 행이다.
_비목아님 = {
...
_주관기관_비목 = {
    # 어휘집 층=사업별·적용대상=주관기관. 창업기업 10종에 없다.
...
def 정본_비목_enum() -> set[str]:
    """`_비목_어휘집.json` 의 guided_json_enum 10종. 비목 문자열의 유일한 정본."""
    import json
...
def 정본비목(원문: str, 정본: set[str] | None = None) -> tuple[str | None, str]:
    """CSV `해당 비목` 값 하나 → (정본 enum 또는 None, 분류 태그).
...
def 비목매핑(적용: bool = False) -> None:
    ...
    분류별 = collections.defaultdict(list)   # 태그 -> [(원문, 정본)]
    본 = set()
    행_커버 = 행_부분 = 행_없음 = 0
    for _, items in rows:
