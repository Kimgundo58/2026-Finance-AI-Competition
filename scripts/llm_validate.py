...
위치인자 2개는 계약 그대로다. 아래 `*` 뒤 키워드는 전부 기본값이 있어
`검증(출력, s맵)` 2인자 호출이 그대로 동작한다. 키워드를 둔 이유: 강등 규칙 중
셋(vlm·verified·F스키마)이 DB 사실을 필요로 하는데, 그걸 함수 안에서 직접 조회하면
테스트가 DB 없이는 못 돈다. 기본값은 라이브 DB 조회이고, 테스트는 스텁을 넣는다.

층 B — 해야할일 설명 환각 대조. 키워드 2개를 쓴다:
    f사실   dict | None   B5(F축 협약·집행 현황)의 값. 문자열이 아니라 dict
    프롬프트 str          조립된 B0~B6 전문. 숫자 화이트리스트의 원천
둘 다 안 넘기면 이 층은 잠들어 있다 — 조립기가 넘기기 전까지 무발효이고, 기존
호출은 아무것도 바뀌지 않는다. 새 강등코드를 더할 때는 `decisions_강등코드_check`
CHECK 제약도 같이 늘려야 한다 — 늘리기 전엔 저장이 거부된다.
...
# 경로 표기는 `["F1.정부지원.현금", ...]` 형식이다 — 컬럼 `정부지원_현금` 의
# `_` 가 `.` 이 된다.
...
def _항_추출(본문: str, 항호: str | None) -> tuple[str, str]:
    ...
    자르기지 요약이 아니다 — 반환값은 반드시 본문의 정확한 부분 문자열이다.
    ...
    if not 항호:
        return 본문, "조전체"
    # `①#2` 처럼 몇 번째인지가 붙어 올 수 있다 — 한 조 안에 같은 항 기호가
    # 두 벌 있는 문서가 실재하므로 `find()` 로 첫 번째만 잡으면 안 된다.
    기호, _, 순 = 항호.partition("#")
...
def 체크항목_본문(codes: set[str] | None = None, dsn: str | None = None) -> dict[str, dict]:
    """`corpus.check_items.code` → {항목, 설명}. `인용`과 같은 원칙 — 생성이 아니라 추출.

    해야할일은 LLM 이 code(안정 식별자)만 고르고 항목·설명은 여기서 DB 원문 그대로
    채운다. `codes` 를 주면 그 안으로 좁힌다(호출부가 이미 `체크코드_enum(사업명=...)`
    로 사업 범위를 알기 때문).
    """
    # `사업명`·`비목` 을 같이 싣는다. 둘 다 NULL 인 항목이 모든 지출에 걸리는
    # 공통 필수다(부가세제외·비교견적준비·사업비카드사용·협약기간내집행·거래처증빙수취·
    # 세금계산서수취 6개). 아래 `_공통필수()` 가 쓴다.
    with db.connect(dsn) as conn:
        if codes:
            rows = conn.execute(
...
def _공통필수(체크항목: dict[str, dict]) -> list[str]:
    ...
    이건 LLM 이 판단할 대상이 아니다. 부가세를 빼는 것, 사업비 카드로 긁는 것,
    협약기간 안에 집행하는 것은 지출 종류와 무관하게 항상 해당한다. LLM 이 "해당하는
    게 없으면 빈 배열" 지시를 곧이곧대로 적용해 빈손으로 끝내면 화면의 「결제 전
    확인」이 통째로 빌 수 있다 — 그래서 서버가 필수 항목을 DB 사실로 붙인다.
    모델의 선택은 여기에 더하는 것이다.
    """
...
def _항호_표시(항호: str | None) -> str | None:
    ...
    `corpus.chunks.항호` 는 값이 세 갈래다 — None(대다수), "①"·"③" 같은 항기호
    (사용자에게 유용하다), "항1~③"·"-4#1" 같은 청킹 내부 좌표. 세 번째는 사용자가
    읽을 표기가 아니라서 버린다 — 내부 좌표를 사람 말로 바꿀 방법이 없고, 억지로
    바꾸면 없는 정보를 지어내는 것이다. 원문은 `원문` 필드가 이미 나르므로 근거가
    사라지지는 않는다.
    """
    if not 항호:
        return None
...
def _조제목_표시(조번호: str | None, 조제목: str | None) -> str | None:
    """조제목 앞에 붙은 조번호 되풀이를 뗀다.

    조번호="붙임2" · 조제목="【붙임 2】창업기업등 사업비 비목 해설표" 처럼 원본이
    이미 중복이면 프론트가 둘을 이어 붙일 때 「붙임2 【붙임 2】…」로 두 번 찍힌다.
    조번호는 위치, 조제목은 이름이라 둘 다 필요하다 — 그래서 조제목 쪽의 되풀이만
    뗀다(조번호를 지우면 위치를 잃는다).
    """
    if not 조제목:
        return 조제목
...
def _조번호_표시(조번호: str | None) -> str | None:
    """`단락035` 처럼 파서 산출물인 조번호를 사용자 표기로 바꾼다.

    원본에 제N조 구조가 없는 문서(안내문·양식)는 `split_articles()` 의 3순위
    단락분할로 떨어지고, 그 결과가 `단락035` 다 — 파싱은 정상인데 그 내부 표기가
    화면 「적용 근거」에 그대로 찍히면 사용자가 못 읽는다. 숫자는 살린다 —
    사용자가 원문에서 위치를 찾는 단서라 지우면 손실이다.
    """
    if not 조번호:
        return 조번호
...
def s번호_메타(s맵: dict, dsn: str | None = None) -> dict[str, dict]:
    ...
    `인용[].조번호·원문 | 코드 | S번호 → DB 원문 치환` 을 여기서 한다.
    ...
    with db.connect(dsn) as conn:
        # chunk — 청크가 항상 항 단위인 건 아니다. 900토큰 미만이면 조 전체가 한
        # 청크로 남는데(청킹 설계상 정상), 그런 조는 항호=NULL 인 청크 한 행이고
        # 조립기(`assemble_context.분해`)는 그 본문을 ①②③… 으로 갈라 S번호를 여러
        # 개 만든다. 여기서 항호를 안 보고 `c.text` 를 통째로 쓰면 같은 조 전문이
        # 항 개수만큼 복제돼 인용 표시·저장이 부풀어 오른다(프롬프트 토큰은 안 늘어난다
        # — 조립기는 `분해()` 로 조각을 정확히 싣는다). `article` 분기와 같게 s맵의
        # 항호로 잘라낸다 — 청크가 이미 한 항이면 `_항_추출` 이 그 항을 그대로
        # 돌려주므로 기존 동작과 같다.
        if 청크:
        ...
        # `기관id` 자리에는 `org_id` 를 넣는다 — 멀티테넌시 방어의 대조 대상이다.
        if l3:
            # `l3_documents.원본파일명` 을 같이 읽는다. `l3_articles.doc_id` 는 UUID 라
            # 그대로 내보내면 화면 「적용 근거」에 UUID 문자열이 그대로 찍힌다. L1·L2 는
            # doc_id 가 사람이 읽는 제목이라 이 문제가 없고, L3 만 어긋나 있었다.
            m = {r[0]: r for r in conn.execute("""
...
def 사고기록(종류: str, 상세: dict, *, decision_id=None, org_id=None,
    ...
    테이블이 아직 없을 수 있다. 그때는 stderr 로만 남긴다 — 사고를 조용히 삼키는
    경로는 만들지 않는다.
    """
...
# 층 B — 해야할일 설명 환각 대조. 여기서 막는 건 "없는 상태" 와 "없는 숫자" 다.
# 인용(S번호)과 항목(code)은 이미 위에서 폐쇄 목록으로 닫혀 있는데 `해야할일[].설명`
# 만 아무 검사 없이 화면으로 나갔다.
#
# 처분은 항목 폐기가 아니라 설명 떨어뜨리기다. 항목과 code 는 남긴다 — 화면은
# code 로 `check_items` 마스터의 정적 설명을 조회할 수 있으므로, 설명이 비면 판정
# 이전의 고정 문구로 대체한다. 항목까지 지우면 룰이 고른 확인항목이 사라져 오히려
# 위험해진다 — 설명이 나쁜 것과 항목이 틀린 것은 다르다.
#
    ...
    if 허용숫자 is not None:
        ...
        if 밖:
            위반.append(("TASK_NUMBER_UNSOURCED",
                        ...
                        "설명"))
    # 4. 근거 S번호 — 층 A 가 붙어 `근거S` 필드가 생기면 자동 발효한다.
...
def 검증(llm출력: dict, s맵: dict, *,
    ...
    # 2. 인용 S번호 — 매핑표에 없는 S번호 인용은 폐기(환각 인용 차단)
    인용목록: list[인용] = []
    인용층: list[str] = []
    누수: list[dict] = []
    for s in llm출력.get("인용") or []:
        ...
        if m is None:
            ...
            continue
        # 멀티테넌시 방어. NULL 은 공용 규범(L1·L2)이라 정상이다.
    ...
    for p in llm출력.get("전제") or []:
        ...
        if 밖:
            ...
            미매핑.extend(밖)
        # `인용`과 같은 s맵 체계이니 같은 방식으로 문서·원문을 채운다. DB 미스는
        # `인용`처럼 폐기하지 않는다 — `사실` 자체는 여전히 유효한 정보라서다.
        m = 메타.get(근거)
        if m is None:
    ...
    # `documents.extraction='vlm'` = 스캔 판독본. A등급 인용 금지 대상이다
    ...
    # 끊긴 참조는 업로드 시점에 알리는 게 원칙이고, 여기는 마지막 그물이다.
    # 조가 지정된 끊긴 참조만 센다 — 조 없는 인용(「국민건강보험법」처럼 문서를
    # 통째로 가리키는 것)은 애초에 펴지 않기로 한 것이다. 그걸로 강등하면 상시
    # 경보가 되고, "근거 불완전" 이 기본 상태가 되면 강등코드가 신호를 잃는다.
    ...
    # 5-d. '가능' 인데 인용이 L3 단독
    # 주관기관 규정은 국가 지침 위에 얹는 문서라 구조적으로 자족하지 않는다 —
    # "총장 승인만 받으면 됩니다" 만 보고 답하면 상위의 "범용성 기자재 소명 필요"
    # 를 놓친 틀린 '가능' 이 된다.
    ...
    # 5-e. precedence 재적용
    # L3 가 항상 이기는 게 아니다 — 여러 사업이 적용범위 조에서 L2 > L3 를 명시한다
    # (L3 가 더 엄격해도 진다). L3 단독 근거로 선 "불가" 가 그 규칙에 걸리면 뒤집힌다.
    #
    # 다만 뒤집는 방향이 관대해지는 쪽이라 그대로 '가능' 까지 올리지 않는다 —
    # 올리려면 L1·L2 조문 인용이 있어야 하는데 그게 없어서 여기 온 것이다.
    # 최대 '조건부' 까지만. 오답 비대칭을 여기서도 지킨다.
    # `적용층` 은 `L1+L2` 같은 결합값이 될 수 있다. 층으로 dict/if 분기하면 결합값에서
    # 조용히 빗나간다 — 분기는 `적용층_기여`(리스트)를 본다. 없으면 옛 계약대로
    # `적용층` 을 쪼개 쓴다.
    ...
    # `프롬프트` 가 비면 숫자 대조는 발효하지 않는다 — 조립기가 넘기기 전까지
    # 잠들어 있고, 넘기는 순간 켜진다.
    ...
    for h in (llm출력.get("해야할일") or []):
        ...
        c = h.get("code")
        if 허용코드 is not None:
            # `체크코드` 가 있는 호출은 스키마가 이제 "code" 하나만 요구한다
            # (항목·설명은 LLM 이 낼 수조차 없다, additionalProperties=False).
            # 항목·설명은 여기서 DB(check_items) 원문 그대로 채운다 — `인용`과
            # 같은 원칙이라 층B 환각검사(_설명검사) 대상이 아니다. LLM 이 쓰지도
            # 않은 텍스트를 "환각인가" 로 재판하는 건 범주 오류다.
            # `허용코드 is not None` 으로 가른다(`c is not None` 이 아니다) —
            # 호출부가 체크코드 를 준 실전 호출만 새 경로를 타야 한다. 옛 계약
            # (체크코드 미전달)은 code+항목+설명을 한 dict 에 같이 넣어 부르므로,
            # `c is not None` 으로 가르면 옛 계약 호출까지 걸려 층B 회귀가 깨진다.
            ...
            continue
        # 체크코드 미제공(옛 계약) — LLM 이 항목·설명을 직접 쓴다.
    ...
    응답 = 최종응답(
        ...
        참조사슬=[],                      # 오케스트레이터가 `참조사슬` 을 넣는다
        강등사유=사유,
        미매핑전제=sorted(set(미매핑)),
    )
    d = 응답.to_dict()
    # 강등코드는 `최종응답` 데이터클래스에 없어 여기서 키를 얹는다. `tenant.decisions.
    # 강등코드` 로 그대로 가므로 새 코드를 늘릴 땐 `04_agent.sql` 의 CHECK 제약도
    # 같이 늘려야 한다 — 안 그러면 INSERT 가 거부된다.
    d["강등코드"] = 코드
    return d, 사유


def _self_test() -> int:
    """층 B 회귀 — DB 없이 돈다 (메타·f경로 스텁). 커넥션을 하나도 열지 않는다."""
    s맵 = {"S01": ("chunk", 1, None)}
