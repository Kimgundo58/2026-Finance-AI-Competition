...
═══ 층 B — 해야할일 설명 환각 대조 (2026-09-01 추가) ═══
    ...
    ✅ **선행조건 닫힘 (2026-09-03 실측).** 한때 새 강등코드 4종이
       `decisions_강등코드_check` 18종 목록 밖이라 INSERT 가 거부됐다. 지금은 아니다 —
       실 DB CHECK 는 **22종**이고 이 파일의 코드 집합과 **양방향 차집합 0** 이다.
       층 B 는 이미 발효 중이다(run 191 에서 `TASK_STATE_UNSOURCED` 1건 발화).
       🔴 새 코드를 더할 때는 그 CHECK 도 같이 늘려라. 늘리기 전엔 저장이 거부된다.
