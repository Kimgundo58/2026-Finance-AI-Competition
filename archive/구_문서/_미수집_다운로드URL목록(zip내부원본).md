# 미수집 문서 다운로드 URL 목록 (검증 완료, 바이트 다운로드만 미완)

작성: 2026-08-10. 이 세션의 클라우드 샌드박스는 한국 정부·대학 도메인(*.go.kr, *.ac.kr, *.or.kr 등)으로의 직접 다운로드가 네트워크 정책상 차단되어, 아래 문서들은 **존재·내용을 확인했지만 파일 저장은 못 했다.** 일반 네트워크(개인 PC)에서 브라우저나 `curl -L -A "Mozilla/5.0"`로 바로 받을 수 있다.
"내용검증" = 실제 문서 내용까지 확인함. "링크확인" = 파일 존재·제목만 확인함.
